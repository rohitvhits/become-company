...
class API1Controller extends BaseController
{
	...
	protected $patientService,$documentPatientService,$doctorService,$locationMasterService,$thirdPartyPatientMaster,$documentUploadService,$patientServicesRequest,$siteSettingService,$emmacareWebhookService, $insuranceMasterService,$patientWiseServiceRequests="";
	public function __construct(PatientService $patientService, DocumentPatientService $documentPatientService, DoctorService $doctorService,LocationMasterService $locationMasterService,ThirdPartyPatientMasterService $thirdPartyPatientMaster,DocumentUploadService $documentUploadService,PatientServicesRequest $patientServicesRequest,SiteSettingServices $siteSettingService,EmmacareWebhookService $emmacareWebhookService,InsuranceMasterService $insuranceMasterService,PatientWiseServicesRequests $patientWiseServiceRequests)
	{
	...
	public function addPatient(Request $request)
	{
		...
		if ($validator->fails()) {
			...
			$getExistingPatientDetails = $this->patientService->checkForThirdPartyExistingData($request->all(),$checkToken->agency_id);
			$patientId = "";
			...
			if(!empty($serviceIds[0])){
				foreach($serviceIds as $st){
					$details = Master::where('id',$st)->where('master_type_fk',11)->where(DB::raw('LOWER(types)'), strtolower($request->type))->first();
	...
	public function createNewPatient($data){
		...
		$dataArray = array(
			...
			'created_by' => env('API_USER_ID'),
			'priority_code'=>isset($data['priority_code'])?$data['priority_code']:"",
		);
	...
	public function addPatientNewOld(Request $request)
	{
		...
		$sendResponseData1 = json_decode($request->form_data,true);
		
        $sendResponseData =$sendResponseData1['patient_basic_data'];
       
		$priceImage = json_decode($request->file,true);
		...
		if ($validator->fails()) {
			...
			if(isset($getExistingPatientDetails->id)){
				...
				$sendResponseData['priority_code'] =$sendResponseData['priority']['priority_code'];
				$sendResponseData['priority'] =$sendResponseData['priority']['name'];
				
				$emergency_contact_name="";
				$emergency_contact_number = "";
				if(count($sendResponseData['patient_emergency_contacts']) >0){
			...
			$data = array(
				...
				'created_by' => $created_by,
				'service_start_date' => $service_start_date,
				'priority_code'=>$sendResponseData['priority_code']??"",
			);
			...
			if ($insertId) {
				...
				$image = '';
                if (isset($priceImage['filename']) && $priceImage['filename'] !="") {
                    $name = uniqid() . time() . '_' . $priceImage['filename'];
                    ...
                    $destination2 = public_path('patientWriteDocument');
                        file_put_contents($destination1.'/'.$name, $priceImage['content'], FILE_APPEND);
                        file_put_contents($destination2.'/'.$name, $priceImage['content'], FILE_APPEND);
                        $image = $name;
                    }else{
                        $binaryImage = $priceImage['content'];
                        Storage::disk('s3')->put('patientdocument/'.$name,$binaryImage);
                        Storage::disk('s3')->put('patientWriteDocument/'.$name,$binaryImage);
                        $image = $name;
                    }
					...
					$data = array(
						'document_name' =>$priceImage['filename'],
						'attachment' => $image,
						'patient_id' => $patientId,
						...
						'extension'=>$sendResponseData1['extension'],
						'size_in_bytes'=>$sendResponseData1['size_in_bytes'],
					    'pdf_type'=>$sendResponseData1['type'],
					);
	...
	public function addPatientNew(Request $request)
	{
        ...
        $sendResponseData =$sendResponseData1['patient_basic_data'];
		$decodePriority = json_decode($sendResponseData['priority'],true);
		...
		if ($validator->fails()) {
			...
			if(isset($getExistingPatientDetails->id)){
                ...
                $sendResponseData['fu_date'] = $sendResponseData['followup_date'];
				$sendResponseData['diciplin'] = $sendResponseData['discipline'];
				$sendResponseData['priority_code'] =$decodePriority['priority_code'];
				$sendResponseData['priority'] =$decodePriority['name'];
