...
class ApiCallLogReportController extends Controller
{
	...
	public function getAppointmentWiseDataShow(Request $request){
		$query = LogsService::getThirdPartyUserData($request->id);
		if(isset($query->id)){
			$query->new_response = json_encode(unserialize($query->new_response));
		}
		return response()->json(['status' => true, 'msg' => 'Data get successfully', 'data' => $query]);
	}
}
