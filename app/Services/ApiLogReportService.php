...
class ApiLogReportService{

    public function apiLogList()
	{
		$auth = auth()->user();

		$query = ThirdPartyPatientLog::select('ebook.id','title','ebook.type','users.first_name','users.last_name','ebook.created_at')->leftjoin('users',function($join){
