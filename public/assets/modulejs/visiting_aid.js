...
function getBasicDetails(id,agencyId){
    $.ajax({
        ...
        success: function (res) {
            ...
            $('.follow_updated_id').text(res.data.follow_date ? moment(res.data.follow_date).format('MM/DD/YYYY') : 'N/A');
            $('.due_updated_id').text(res.data.due_date ? moment(res.data.due_date).format('MM/DD/YYYY') : 'N/A');
...
$('#third_party_patient_export').on('click', function (e) {
    ...
    $.ajax({
        ...
        success:function(res){
            ...
            link.download = "visitingaids"+_DATE_TIME+".csv";
            link.click();
        }
    });
