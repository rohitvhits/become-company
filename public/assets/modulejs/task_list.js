...
$(document).on("click", "#main_checkBox1", function () {
...
$(document).on("change", ".cbox_id", function () {
    toggleBulkButtons();
});

// Function to show/hide bulk buttons based on selection
function toggleBulkButtons() {
    var selectedCount = $('.cbox_id:checked').length;

    if (selectedCount > 0) {
        $('#bulk-assign-task-btn').show();
        $('#bulk-assign-portal-btn').show();
    } else {
        $('#bulk-assign-task-btn').hide();
        $('#bulk-assign-portal-btn').hide();
    }
}
...
function getChangeStatusById(status) {
    ...
    if (status === 'Completed') {
        $.confirm({
            ...
            content: 'Are you sure you want to mark this task as <strong>Completed</strong>?',
            type: 'green',
            buttons: {
                confirm: {
                    text: 'Yes, Complete',
                    ...
                    action: function() {
                        executeStatusChange(response_id, status);
                        if (typeof getTaskListView === 'function') {
                            getTaskListView();
                        }
        ...
        // For other statuses, execute directly without confirmation
        executeStatusChange(response_id, status);
    }
}

function executeStatusChange(response_id, status) {
    $.ajax({
        type: "GET",
        ...
        success: function (res) {
            ...
            if (res.status == 1) {
                ...
                if (res.data.status == 'Pending') {
                    ...
                    $("#status" + response_id + "").html('<span class="badge badge-warning">Pending</span>');
                    $(".statusBtn").addClass('badge-warning').removeClass('badge-danger badge-success badge-info');
                }
                if (res.data.status == 'Urgent') {
                    $("#status-name").html('Urgent');
                    $("#status" + response_id + "").html('<span class="badge badge-danger">Urgent</span>');
                    $(".statusBtn").addClass('badge-danger').removeClass('badge-warning badge-success badge-info');
                }
                if (res.data.status == 'Outstanding') {
                    ...
                    $(".statusBtn").addClass('badge-success').removeClass('badge-warning badge-danger badge-info');
                }
                if (res.data.status == 'Completed') {
                    $("#status-name").html('Completed');
                    $("#status" + response_id + "").html('<span class="badge badge-info">Completed</span>');
                    $(".statusBtn").addClass('badge-info').removeClass('badge-warning badge-danger badge-success');
                }
                ...
                $.each(data, function (i, v) {
                    response += `<div class="comment-entry">
                                            ...
                                            <div class="comment-meta">
                                                ...
                                                <span class="comment-time"><i class="fa fa-clock-o"></i> ${moment(v.created_date).format('MM/DD/YYYY HH:mm A')}</span>
                                            </div>
            ...
            if (data.length != 0) {
                ...
                $.each(data, function (i, v) {
                    response += `<div class="activity-wrapper" id="activity-wrapper"><div class="active-log-wrapper d-flex align-items-start my-2 py-1">
                                ...
                                <span class="log-info"><span class="action-by bold"> ${v.users.first_name}  ${v.users.last_name} </span><span class="comment-time"><i class="fa fa-clock-o mr-1"></i>${moment(v.created_at).format('MM/DD/YYYY HH:mm A')}</span></span><p class="tx-13 text-muted my-1 bold">${v.description}</p>
                                </div>
                            </div></div>`;
                })
            }
...
function changeDate() {
    ...
    if(due_date == '') {
        ...
        $('#task_due_date_id_error').html("Start Date cannot be later than Due Date.");
        count++;
        return false;  
    }
    if(count == 0) {
...
function flagTaskChange(id) {
    ...
    $.confirm({
        ...
        content: function () {
            ...
            var html = '<b><label for="reason">Reason:</label><b>' +
...
function openModal(response_id) {
    ...
    $.ajax({
        ...
        success: function (datalist) {
            ...
            $("#created-by").text(datalist.data.first_name + " " + datalist.data.last_name);
            ...
            $("#task-assign").text(datalist.data.assignFname + " " + datalist.data.assignLnamae);
            ...
            $("#task_due_date_id").val(moment(datalist.data.due_date).format('MM/DD/YYYY'));
            ...
            if(datalist.data.department != null){
                $('#task-department').html(datalist.data.department.name);
                $('#edit_department_id').val(datalist.data.department.id).change();
            }else{
                $('#task-department').html("No department assigned yet.");
...
function openDeptModal(){
    $('#task_dept_modal').modal('show');
    $('#task_dept_id_error').html("");
    $('#task_dept_id').val($("#task-dept").text());
}

function saveTaskDept(){
    var task_id = $("#hdn_task_id").val();
    var task_dept = $("#edit_department_id").val();
    $.ajax({
        url: TASK_DEPT_UPDATE,
        dataType: "json",
        type: "POST",
        data: {
            'id': task_id,
        ...
        success: function (datalist) {
            toastr.success(datalist.error_msg);
            var task_dept_name = datalist.task_dept_name != "" ?  datalist.task_dept_name : 'No department assigned yet'; 
            $('#task-department').html('<b>'+task_dept_name+'</b>');
            $('#task_dept_modal').modal('hide');
            getLogList();
        }
    });
}
        ...
        success: function (res) {
            if (res.status == 1) {
                ...
                if (res.TaskDetails.priority == 'High') {
                    ...
                    $("#priority" + response_id + "").html('<span class="badge badge-outline-warning">High</span>');
                    $(".prioBtn").addClass('badge-warning').removeClass('badge-danger badge-success badge-info');
                }
                if (res.TaskDetails.priority == 'Medium') {
                    $("#priority-name").html('Medium');
                    $("#priority" + response_id + "").html('<span class="badge badge-outline-danger">Medium</span>');
                    $(".prioBtn").addClass('badge-danger').removeClass('badge-warning badge-success badge-info');
                }
                if (res.TaskDetails.priority == 'Low') {
                    $("#priority-name").html('Low');
                    $("#priority" + response_id + "").html('<span class="badge badge-outline-success">Low</span>');
                    $(".prioBtn").addClass('badge-success').removeClass('badge-warning badge-danger badge-info');
                }
                getLogList();
            } else {
                toastr.error(res.error_msg);
...
function getTaskListView(page=""){
    ...
    var taskDept = $('#fil_department_id').val();
    var visiblity_assign = $('#visiblity_assign').val();
    ...
    if(path == '/tasks/task-list') {
        $.ajax({
            ...
            data: {
                ...
                fil_department_id: taskDept,
                visiblity_assign : visiblity_assign
            },
            success: function (res) {
                $('#task_list').html(res);
...
function taskAdd() {
    ...
    var startDate = $("#task_start_date_id").val();
    ...
    if (task_name_id.trim() == "") {
        ...
        cnt = 1;
        $('#task_name_id_error').html("Task title should be less than 70 characters.");                    
    }
    if (assign_to_id == "") {
    ...
    if (startDate == "") {
        ...
        if (!regex.test(value)) {
            error.innerText = 'Please enter valid start date';
            cnt = 1;
        }
...
$(document).on('click', '.task-list-pagination .pagination a', function (event) {
    $('li').removeClass('active');
    $(this).parent('li').addClass('active');
    event.preventDefault();
    var myurl = $(this).attr('href');
...
function bulkAssignTask() {
    var selectedTasks = [];
    $('.cbox_id:checked').each(function() {
        selectedTasks.push($(this).val());
    });

    if (selectedTasks.length === 0) {
        toastr.error('Please select at least one task to assign.');
        return false;
    }

    var assignUserId = $('#bulk_assign_to_user_select').val();

    if (!assignUserId) {
        $('#bul_assign_user_id_error').text('Please select an assignee.');
        return false;
    }

    $('#bul_assign_user_id_error').text('');

    var assignUserName = $('#bulk_assign_to_user_select option:selected').text();
    var taskCount = selectedTasks.length;

    // Show confirmation dialog
    $.confirm({
        title: 'Confirm Bulk Assignment',
        content: 'Are you sure you want to assign <strong>' + taskCount + ' task(s)</strong> to <strong>' + assignUserName + '</strong>?',
        type: 'orange',
        buttons: {
            confirm: {
                text: 'Yes, Assign',
                ...
                action: function() {
                    // Show loader
                    var $submitBtn = $('#bulk-assign-task .btn-success');
                    var originalText = $submitBtn.text();
                    $submitBtn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> Processing...');

                    $.ajax({
                        type: "POST",
                        url: BULK_ASSIGN_TASK,
                        data: {
                            '_token': CSRF_TOKEN,
                            'task_ids': selectedTasks,
                            'assignUserId': assignUserId,
                            'selectedText': assignUserName
                        },
                        success: function (res) {
                            // Hide loader
                            ...
                            if (res.status == 1) {
                                ...
                                $('#bulk-assign-task').modal('hide');
                                $('.cbox_id').prop('checked', false);
                                $('#main_checkBox1').prop('checked', false);
                                toggleBulkButtons();
                                getTaskListView();
                            } else {
                                toastr.error(res.message || 'Sorry, something went wrong. Please try again.');
                            }
                        },
                        error: function (xhr, status, error) {
                            // Hide loader
...
function bulkAssignPortal() {
    ...
    var portalId = $('#bulk_portal_id').val();

    if (!portalId || portalId <= 0) {
        $('#bulk_portal_id_error').text('Please enter a valid Patient/Portal ID.');
        return false;
    }

    $('#bulk_portal_id_error').text('');

    var taskCount = selectedTasks.length;

    // Show confirmation dialog
    $.confirm({
        title: 'Confirm Bulk Portal Assignment',
        content: 'Are you sure you want to assign <strong>' + taskCount + ' task(s)</strong> to Patient/Portal <strong>#' + portalId + '</strong>?',
        type: 'orange',
        buttons: {
            confirm: {
                text: 'Yes, Assign',
                ...
                action: function() {
                    // Show loader
                    var $submitBtn = $('#bulk-assign-portal .btn-success');
                    var originalText = $submitBtn.text();
                    $submitBtn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> Processing...');

                    $.ajax({
                        type: "POST",
                        url: BULK_ASSIGN_PORTAL,
                        data: {
                            '_token': CSRF_TOKEN,
                            'task_ids': selectedTasks,
                            'portal_id': portalId
...
$(document).on("change", "#visiblity_assign", function () {
    getTaskListView(1);
});
...
$(document).on('click', '.patient-task-list-pagination .pagination a', function (event) {
    $('li').removeClass('active');
    $(this).parent('li').addClass('active');
    event.preventDefault();
    var page = $(this).attr('href').split('page=')[1];
        ...
        success: function (res) {
            ...
            if (res.status === true) {
                ...
                $.each(res.data, function (index, dept) {
                    ...
                    if(dept.id == _ASSIGN_DEPT_ID){
                        selected_dept_name = dept.name;
                    }
                });
                $('#department_id').html(html);
                $('#edit_department_id').html(html);
                $('.assign_dept').html(html);
                $("#assign_department").html(selected_dept_name);
                $("#assign_dept").val(_ASSIGN_DEPT_ID);
            } else {
...
$('#exampleModal-task').on('show.bs.modal', function () {
    fillDepartment();
});
