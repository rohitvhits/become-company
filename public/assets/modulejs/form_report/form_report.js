...
if (typeof _FORM_REPORT_LIST !== 'undefined') {
    loadEsignReportList(1);
}
...
function loadEsignReportList(page) {
    ...
    $('#loadertag1').removeClass('hide');
    $.ajax({
        url: _FORM_REPORT_LIST + "?page=" + page,
        type: "get",
        data: formsubmit,
...
function refresh() {
    ...
    $('#mark_as_completed_by').tokenInput("clear");
...
function exportCsv() {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            link.download = "form_report" + _DATE_TIME + ".csv";
            link.click();
        }
    });
}
...
var markAsCompletedById = '';
var markAsCompletedByName = '';
$("#mark_as_completed_by").tokenInput(urlUserToken, {

    tokenLimit: 1,
    zindex: 9999,
    prePopulate: markAsCompletedById !== "" && markAsCompletedByName !== "" ? [{
        id: markAsCompletedById,
        name: markAsCompletedByName
    }] : [],
    onAdd: function(item) {
        $('#mark_as_completed_by_id').val(item.id);
        $('#mark_as_completed_by_name').val(item.name);
    },
    onDelete: function(item) {
        $('#mark_as_completed_by_id').val('');
        $('#mark_as_completed_by_name').val('');
    }
});