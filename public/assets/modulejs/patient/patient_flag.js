...
$(function() {
    ...
    $('.dob').daterangepicker({
        ...
        $('.dob').val(chosen_date.format('MM/DD/YYYY') + ' - ' + end_date.format(
            'MM/DD/YYYY'));
    })
});

function loadAppointmentFlagList(page) {
    var formsubmit = $('#formsubmit').serialize();
    $('#loaderDashboardGraph').attr('style','display:flex');
    $('#resp').html("")
    $.ajax({
        url: _FLAG_APPOINTMENT_LIST + "?page=" + page,
        type: "get",
        data: formsubmit,
...
function makeFlagRead(id,record_id,type){
    $.ajax({
        ...
        success: function (response) {
           var urlRedirection = '/patient/view/'+record_id;
           if(type == 'Task'){
                urlRedirection = '/tasks/task-list/'+record_id;
           }
