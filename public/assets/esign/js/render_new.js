...
function newRanderPage(num){
    ...
    if(pageNum ==pdfDoc?.numPages){ 
		...
		$('#previousid').attr('style',"display:block;");
		$('#finish_id').attr('style',"display:block");
		...
		$('#nextid').attr('style',"display:block");
		$('#previousid').attr('style',"display:block;");
		$('#finish_id').attr('style',"display:none");
	}else{   
		if(pdfDoc?.numPages >1){
	...
	if(removeScript !=''){
		if(typeof maximumPgaes !="undefined" &&  maximumPgaes ==pageNum ){ 
				$('#finish_id').attr('style',"display:block");
		}
	}
	$("#page_num").text(num);
 ...
 */
 ...
 function generatePdfPage(pageNumber) {
