...
$(".eyeIcon").click(function(){
	const datetimeValue = $(this).data('datetime');
	const id = $(this).data('id');
	
	inviting = false;
	enableTools();
	launchEditor();
	GetLoadHistoryComponents(id);
});
...
function launchEditor() {
	...
	$('.version-class').attr('style','display:none');
	$('.version-history-card').attr('style','display:none');
	renderPage(pageNum);
};
...
function assemble(data, prepare) {
	...
	if (data.type === "rotate") {
		...
		if (data.group === "field") {
			...
			var tttttt = '<action id="' + data.id + '" type="' + data.type + '" group="' + data.group + '" page="' + data.page + '" xPos="' + data.xPos + '" yPos="' + data.yPos + '" width="' + data.width + '" height="' + data.height + '" image="' + data.image + '" value="' + new_array + '" datats="' + data.type + '" required="' + data.required + '" readOnly="' + data.readOnly + '" background_color="' + data.background_color + '"  signer_id="' + data.signer_id + '" manual_height="' + data.manual_height + '"> ';
			$(".signer-assembler").append(tttttt);

		} else if (data.group === "input") {

			$(".signer-assembler").append('<action type="' + data.type + '" group="' + data.group + '" page="' + data.page + '" xPos="' + data.xPos + '" yPos="' + data.yPos + '" width="' + data.width + '" height="' + data.height + '" required="' + data.required + '" readOnly="' + data.readOnly + '" manual_height="' + data.manual_height + '">');
		}
...
function orgnizeData(prepare) {
	...
	$('.signer-builder .signer-element').each(function (index, value) {
		...
		if (actionType === "image" || actionType === "signature" || actionType === "symbol" || actionType === "shape") {
			...
			fieldText = $(textHolder[0]).attr("field_value");
			...
			assemble({'normalValueRadio': eid[0].value, "id": signerElement.attr('id'), "required": eid[0].required, "readOnly": eid[0].readOnly, "InputId": eid[0].id, "group": group, "type": actionType, "page": pageNumber, "xPos": elementXpos, "yPos": elementYpos, "width": elementWidth, "height": elementHeight, "text": actionType, "value": checkedval, 'name': eid[0].name, "background_color": $(eid[0]).attr("background_color"), "signer_id": $(eid[0]).attr("signer_id"), 'datas_keys': $(eid[0]).attr("data-keys"), "group": $(eid[0]).attr("group"), "checkBoxsTemp": $(eid[0]).attr("tempids"), 'groupname': $(eid[0]).attr("groupsName") }, false);
...
function prepareData(save) {
	...
	$('.signer-assembler action').each(function (index, value) {
		...
		var selectedObjectVal = [];
		var dynamicDropdownIdVal = "";
		...
		var manualHeight = '';
		if(typeof $(this).attr('manual_height') !="undefined"){
			manualHeight = $(this).attr('manual_height');
		}
		actions.push({
			id: $(this).attr('id'),
			type: $(this).attr('type'),
	...
	swal({
		...
		if (isConfirm == false) {
			...
			$.ajax({
				...
				type: "POST",
				data: {
					"actions": actions, "_token": tokens, "docWidth": $("#document-viewer").width(), "document_key": document_key, "signing_key": signingKey,
				},
				success: function (response) {

					if (response == 1) {
						swal("Done!", "Template successfully updated.", "success");
						setTimeout(function () { window.location.href = baseUrl + "/document-new?id=" + document_key; }, 2000);
					} else {
...
var enabledDisabledDrag = false;
var sign;
$('.signstatus').click(function () {
	if(!enabledDisabledDrag){
...
$('.stampstatus').click(function () {
	if(!enabledDisabledDrag){
		...
		$('<div class="signer-element selected-element tttts" resizeable="true" status="drop" type="stamp" id="stamp_' + stamp + '" page="' + pageNum + '"><img src="' + urls + 'assets/images/stamp.jpg" class="img_wrap" id="stamp_signer_' + stamp + '"></div>').appendTo(".signer-builder");
		$(document).mousemove(function (event) {
			$('#tempids').val(1);
			$('.prev').val('stamp');
			$(".signer-element[status=drop]").css({ left: event.pageX + 1, top: event.pageY + 1 });
...
$('.dropdowsns').click(function () {
	if(!enabledDisabledDrag){
		...
		$('<div class="signer-element ui-resizable resizable-container" tempId="Dropgroup_' + drops + '" type="dropdown" page="' + pageNum + '" status="drop" id="dropdowsns_' + drops + '"><select  groupsName="dropdowngroup' + drops + '" tempIds="dropdowngroup_' + drops + '" id="dropid' + drops + '" class="drips" style=""><option>select</option></select></div>').appendTo(".signer-builder");
...
$(document).on("mouseup", ".resizable-container", function () {
...
function initElementsResize() {
	$(".signer-element[resizeable=true]").resizable({
		...
		resize: function (event, ui) {
			...
			ui.helper.find("img").width(ui.size.width);
			ui.helper.find("img").height(ui.size.height);
...
function showTemplateFields() { //	
	if (isTemplate === "Yes" && templateFields !== '' && $("body").hasClass("editor") && $(".signer-builder").is(':empty')) {
		...
		var signaturesID = [];
		var stampsID = [];
		var checkboxFieldsIDS = [];
		var radioFieldsIDS = [];
		var dropdownFieldsIDS = [];
		...
		$.each(templateFields, function (i, field) {
			...
			if (field.type == "image") {
				if(!signaturesID.includes(field.id)){
					...
					var height = ''
					if(typeof field.manualHeight !="undefined"){
						height = field.manualHeight;
					}

					if(field.width =='32' && field.height =='32'){
						field.width = 100;
						field.height = field.width * 0.363636;
					}else{
field.width = parseFloat(signerScale(Width));
field.height= parseFloat(signerScale(height));
					}
					
					$('<div class="signer-element " resizeable="true" id="' + field.id + '" type="image" group="field" page="' + field.page + '" style="display:none;left:' + xPos + 'px;top:' + yPos + 'px;position:absolute;width:' + field.width + 'px;height:' + field.height + 'px;"><img src="' + field.image + '" class="img_wrap"  id="' + ids + '" style="width:' + field.width + 'px;background-color:' + field.background_color + ';" background_color="' + field.background_color + '" signer_id="' + field.signer_id + '" data-height="'+height+'"></div>').appendTo(".signer-builder");
					initElementsDrag();
					signaturesID.push(field.id);
				}
				
			}else if (field.type == "stamp") {
				if(!stampsID.includes(field.id)){
					splitid = field.id.split('_');
					if (splitid[0] == 'stamp') {
					...
					initElementsDrag();
					stampsID.push(field.id)
				}
				
			} else if (field.type == "text") {
				if(!textBoxIds.includes(field.id)){
					clsaass = '';
						...
						if (splitid[0] != 'staff' && splitid[0] != 'caregiver') {
							...
							if (splitid[0] === 'datesigned') {
								...
								if(field.fieldText != "undefined"){
									field_value =field.fieldText;
								}
	
								$('<div class="signer-element" tempid="' + field.groupSmapleId + '" id="' + field.id + '" type="text"    page="' + field.page + '" style="position:absolute;display:none;left:' + xPos + 'px;top:' + yPos + 'px;" ><textarea heightId="' + field.textsmall + '" groupsName="' + field.groupNames + '" type="text" id ="' + textid + '"type="' + field.type + '"  placeHolder="' + place.trim() + ' "class="writing-pad1  ' + classes + ' ' + clsaass + '" style="width:' + Width + 'px;height:' + height + 'px; background-color:' + field.background_color + ';font-size:' + font + '" value="' + vtext + '" ' + readonly + ' ' + required + '  background_color="' + field.background_color + '" signer_id="' + field.signer_id + '" title="' + field.temp3 + '" font="' + font + '" datakey="' + field.temp1 + '" field_value="'+field_value+'"></textarea> </div>').appendTo(".signer-builder");
							}
						}
					}
					textBoxIds.push(field.id);
				}
			} else if (field.type == "dropdown") {
				if(!dropdownFieldsIDS.includes(field.id)){
					...
					$('<div class="signer-element resizable-container"   id="' + field.id + '" type="dropdown"  tempid="' + field.groupSmapleId + '"   page="' + field.page + '" style="position:absolute;display:none;left:' + xPos + 'px;top:' + yPos + 'px;" ><select  groupsName="' + field.groupNames + '" tempIds="' + field.groupSmapleId + '" name="dropdownd" id="dropid' + explode[1] + '"  class="drips ' + classes + '" style="height:' + field.height + 'px; width:' + field.width + 'px; background-color:' + field.background_color + ';" background_color="' + field.background_color + '" signer_id="' + field.signer_id + '" ' + requireds + ' ' + readonly + ' >' + dropsdownList + '</select></div>').appendTo(".signer-builder");
	
					dropdownFieldsIDS.push(field.id);
				}
				...
				if(!checkboxFieldsIDS.includes(field.id)){
					if (field.checked == 1) {
					...
					$('<div class="signer-element ui-draggable ui-draggable-handle"  tempid="' + field.groupSmapleId + '" id="' + field.id + '" type="checkbox"  group="input"   page="' + field.page + '"  style="position:absolute;display:none;left:' + xPos + 'px;top:' + yPos + 'px;' + colors + '"><input type="checkbox" class="checkbox_wrapper" contenteditable="true"  name="' + field.name + '" style="margin-top:-2px !important; line-height:unset !important"  value="' + field.normalValue + '" ' + checked + ' id="' + field.bold + '"  background_color="' + field.background_color + '" signer_id="' + field.signer_id + '" ' + required + ' data-keys="' + field.addmore + '" group="' + field.groups_checkbox + '" ' + readonly + ' groupsName="' + field.groupNames + '" tempIds="' + field.groupSmapleId + '">' + onclicks + '</div>').appendTo(".signer-builder");
					...
					checkboxFieldsIDS.push(field.id);
				}
				
			}
			else if (field.type == "radio") {
				if(!radioFieldsIDS.includes(field.id)){
					var explode = field.id.split('_');
		...
		$.each(templateFields, function (i, field) {
			...
			if (field.type == "image") {
				...
				if (docusignId == field.signer_id) {
					...
					$('<div class="signer-element resizable-container" id="' + field.id + '"   type="text"    page="' + field.page + '" style="position:absolute;display:none;z-index:99;left:' + xPos + 'px;top:' + yPos + 'px;" ><select name="dropdownd" id="dropid' + explode[1] + '" ' + HideShow + ' style="' + bgrequired + ';height:' + height + 'px;width:' + Width + 'px;background-color:' + field.background_color + '" background_color="' + field.background_color + '" signer_id="' + field.signer_id + '">' + dropsdownList + '</select></div>').appendTo(".signer-builder");
...
function getKeys(vals) {
	...
	$('#checks' + vals).attr('placeHolder',textarea1);
}
...
function getGenerateArray(key, val, temp = null) {
	...
	if (key == 'signature') {
		...
		var heightValue = $('#signatures_signer_'+val).attr('data-height');
		var height = ''
	...
	if (key == 'stamp') {
		...
		html = "<div class='row' id=" + val + "><div class='box box-solid'><div class='box-body'><div id='" + val + "'><div class='form-group'> <label>Stamp</label></div><hr><div class='col-md-12'><div class='form-group'><label>Signer</label><select name='changesColor' onchange='setSigner(this.id,\"stamp_signer_" + val + "\"," + stampname + "," + val + ");' id='signer_stamp" + val + "' class='form-control'>" + selected + "</select></div><hr></div><div class='col-md-12'><div class='form-group'><label>Data Label</label><div class=''>Stamp " + val + "</div></div><hr></div><div class='col-md-12'><div class='form-group'><label>" + required + "</label>Required Field</div></div><div class='col-md-12'><div class='form-group'><label>" + readOnly + "</label>Read Only</div></div></div></div><hr><a style='margin-left:10px' href='javascript:void(0)' onclick='getDeleteFields(" + did + "," + deleteids + ")'>Delete</a></div></div></div>";
	}
	...
	if (key == 'text') {
		...
		var fieldText = $('#checks' + val).attr('field_value');
		...
		fieldDetails = "";
		if (typeof fieldText !== "undefined" && fieldText !== "undefined" && fieldText !== "") {
			fieldDetails = fieldText;
		}
		var temps = '';
		if (temp == null) {
			font = '10';
	...
	if (key == 'checkbox' || key == "checkboxs") {
		...
		html = "<div class='row' id='" + val + "'><div class='box box-solid'><div class='box-body'><div id='" + val + "'><div class='form-group'><i class='fa fa-check-square-o' aria-hidden='true'></i>&nbsp;&nbsp;<label>Checkbox Group</label></div><hr><div class='col-md-12'><div class='form-group'><label>Signer</label><select name='changesColor' onchange='setSigner(this.id,\"" + updatenewselecte.groupname + "\"," + signname + "," + val + ");' id='signer_" + groupname + "' class='form-control'>" + selected + "</select></div></div><div class='col-md-12'><div class='form-group'><label>Data Label</label> <div class=''> Checkbox " + val + "</div></div><hr></div><div class='col-md-12'><div class=''><div class='form-group'><label>" + requiredTitle + "</label><span style='margin-left: 4px;'>Required Field</span></div></div><div class=''><div class='form-group'><label>" + readOnlyTitle + "</label><span style='margin-left: 4px;'>Read Only</span></div></div><hr></div><div class='col-md-12'><div class='form-group'><label>Checkbox Value</label><div class='chkboxval" + val + "'>" + checkBoxYsnamic + "</div></div><hr></div>" + ConditionRules + "<div class='col-md-12'><div class='form-group'><label>Conditional Logic</label><div classs=''><button onclick='showHeaders(" + signname + "," + val + "," + setText + ")'> Create Rules</button></div></div><hr></div>" + SenderTests + "" + AssignTests + "<a href='javascript:void(0)' onclick='getDeleteFields(" + localid + "," + deleteids + ")' style='margin-left: 14px;'>Delete</a></div></div></div>";
	...
	if (key == 'datesigned') {
		...
		html = "<div class='row' id=" + val + "><div class='box box-solid'><div class='box-body'><div id='" + val + "'><div class='form-group'><label>Date Signed</label></div><hr><a href='javascript:void(0)' onclick='getDeleteFields(" + localid + "," + deleteids + ")'>Delete</a></div></div></div>";
	}
...
function GetLoadHistoryComponents(id) {
	$.ajax({
		url: baseUrl + "/template/esign-lookup-history-fields/" + document_key + "?id=" + id,
		type: "GET",
...
function openSpan(template_id,created_date) 
{
	$("#right-sidebar").toggleClass("open");
	$('#loader').show();
	var tableBody = $('#filtered-users-table');
    tableBody.empty();
    $('.total-record').text('0'); 

	$.ajax({
		url: _TEMPLATE_USER_URL,  
		method: 'GET',
		data: {
		...
		success: function(response) {   
			$('#loader').hide();             
			updateFilteredUsersTable(response,created_date);
		},
...
function updateFilteredUsersTable(data,created_date) {
	
	var tableBody = $('#filtered-users-table');
	tableBody.empty();
	let totalCount = 0;

	if (data.filteredTemplateUsers.length > 0) {
		var i = 1;
		$.each(data.filteredTemplateUsers, function(index, val) {

			$.each(val.users, function(userIndex, user) {
			if(val.current_date == created_date) {
				totalCount++;
				var statusLabel = '';
				var row = '<tr>';
				if (user.status === 'Pending') {
					statusLabel = '<span class="badge badge-outline-warning">Pending</span>';
				} else if (user.status === 'Completed') {
					statusLabel = '<span class="badge badge-outline-success">Completed</span>';
				}

				row += '<td>' + i++ + '</td>';
				row += '<td>' + (user.sender_name || '-') + '</td>';
				row += '<td>' + (user.receipt_name ? '<a href="/patient/view/' + user.main_intakeId + '" target="_blank">' + user.receipt_name + '</a>' : '-') + '</td>';
				row += '<td>' + (user.type || '-') + '</td>';
				row += '<td>' + statusLabel + '</td>';
				row += '</tr>';
				tableBody.append(row);

				$('.total-record').text(totalCount);
			}
		});
		...
		if (totalCount === 0) {
			$('.total-record').text('0');
			tableBody.append('<tr><td colspan="6" class="text-center">No record available</td></tr>');
		}
}
...
$(document).ready(function() {
    $('#search-input').on('keyup', function() {
        ...
        $('#filtered-users-table tr').filter(function() {
            var name = $(this).find('td:eq(1)').text().toLowerCase();
            var type = $(this).find('td:eq(2)').text().toLowerCase();
            var status = $(this).find('td:eq(3)').text().toLowerCase();

            var isVisible = name.includes(searchValue) || type.includes(searchValue) || status.includes(searchValue);
        ...
        $('.total-record').text(visibleCount);
    });
});

function getKeysDefualt(val){
	var value = $('.fieldsTextValue'+val).val();
	var text = $('#checks'+val).attr('field_value',value);
}

function addHeight(vals) {

	var textarea1 = $(".height_sign" + vals).val();
	$('#signatures_signer_' + vals).attr('data-height',textarea1);
