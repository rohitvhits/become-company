  ...
  $('.signer-assembler action').each(function (index, value) {
    ...
    const result = globalNewResponse.find(item => item.id === $(this).attr('id'));
      ...
      if(typeof result != 'undefined'){
          ...
          let updatedSelectTypes=0;
          if(typeof $(this).attr('updatedSelectType') !='undefined'){
            updatedSelectTypes = $(this).attr('updatedSelectType');
          }

          result.updatedSelectType = updatedSelectTypes;
...
function showTemplateFields() { //  
    ...
    if (isTemplate === "docusing" && templateFields !== '' && $("body").hasClass("editor") && $(".signer-builder").is(':empty')) {
      ...
      $.each(templateFields, function (i, field) {
        ...
        if(typeof _VIEW_ESIGN_TYPE != 'undefined' && field.signer_id !="undefined"){
          permissionFlag = 0;
          if(docusignId.includes(field.signer_id?.toLowerCase())){
            permissionFlag = 1;
          }
        }
        ...
        if(permissionFlag ==1){
          ...
          if (field.type == "image") {
            ...
            var str = field.image.replace(/\n/g, '');
            var dataids = 0;
            if(field.text !=""){
              str = field.text.replace(/\n/g, '');
              dataids = 1;
            }
            ...
            $('<div class="signer-element" onclick="mySign(' + cnt + ')"   type="image" page="' + field.page + '" style="left:' + xPos + 'px;top:' + yPos + 'px;position:absolute;display:none;" id="' + field.id + '" dataid="' + cnt + '"  page="' + pageNum + '"><img src="' + str + '" id="img' + cnt + '" style="height:'+height+'px;width:'+width+'px;" background_color="" signer_id="' + field.signer_id + '" data-flag="'+field.updatedSelectType+'" dataids="'+dataids+'"></div>').appendTo(".signer-builder");

          }else if (field.type == "stamp") {
              resizeElements = 1;
              field.xPos = (parseFloat(signerScaleStamp(field.xPos,'width')));
              field.yPos = (parseFloat(signerScaleStamp(field.yPos,'height')));
              ...
              $('<div onclick="myStamp(' + cnt + ')" class="signer-element selected-element stamps"  status="drop"  type="stamp" page="' + field.page + '" style="left:'+field.xPos+'px;top:'+field.yPos+'px;position:absolute;display:none;" id="' + field.id + '" dataid="' + cnt + '"  page="' + pageNum + '"><img src="' + str + '" id="img' + cnt + '" style="height:'+field.height+'px;width:'+field.width+'px;" background_color="" signer_id="' + field.signer_id + '" data-flag="0" dataids="'+dataids+'"></div>').appendTo(".signer-builder");
              makeStampResizable(cnt);
              ...
              if (field.temp1 != '' && field.temp3 != '') {
                ...
                $('<div  class="signer-element" type="text" group="input" page="' + field.page + '" style="position:absolute;display:none;left:' + xPos + 'px;top:' + yPos + 'px;z-index:99;"><textarea type="text" id="' + field.id + '" dataid="' + field.temp1 + '" value="'+field.text+'" class="writing-pad1" style="font-size:' + fonts + 'px;' + marigns + ';width:' + widthss + '; ' + word_break + '" value="'+field.text+'">'+field.text+'</textarea></div>').appendTo(".signer-builder");
                ...
                if (explode[0] == 'datesigned') {  
                    ...
                    console.log(field);
                    if(field.text !=""){
                      DateSingDate = field.text;
                    }
                    $('<div id="' + field.id + '" class="signer-element" type="text"  group="input"   page="' + field.page + '" style="position:absolute;display:none;left:' + xPos + 'px;top:' + yPos + 'px;"><textarea  type="text"   placeHolder="' + field.placeHolder + '" class="writing-pad1 ' + addClasss + '" id="' + field.id + '" style="width:' + signerScale(Width) + 'px;height:' + height + 'px;color:' + field.color + ';font-size:' + field.font + 'px;font-family:' + field.fontfamily + ';color:' + field.color + ';' + colors + '" value="' + DateSingDate + '" contenteditable="true" >' + DateSingDate + '</textarea></div>').appendTo(".signer-builder");
                }
            ...
            $('<div class="signer-element ' + field.groupNames + '" id="' + field.id + '" type="radio" groupName="' + field.groupNames + '" group="input"   page="' + field.page + '"  style="' + bgrequired + ';position:absolute;left:' + xPos + 'px;top:' + yPos + 'px;"><input onchange="getChange(' + checkboxid + ')" ' + checked + ' ' + disabled + ' type="radio" id = "' + field.bold + '" class="radio_wrap"  value="' + field.checked + '" ' + HideShow + '  style="' + inlineStyles + '"  name="' + field.name + '" ' + readonly + ' ></div>').appendTo(".signer-builder");
            $.each(tempHeaderForHide[0], function (i, kes) {
...
function GetLoadComponents() {
  ...
  // $.ajax({
  //   url: baseUrl + "/template/esign-lookup-fields-new1/" + document_key,
  //   type: "GET",
...
function signerScaleStamp(dimesion,type){
  templateWidth = $("#document-viewer").offset();
  if(type =='width'){
    dimesion = parseFloat(dimesion) + parseFloat(templateWidth.left);
  }else{
    dimesion = parseFloat(dimesion) + parseFloat(templateWidth.top)
  }
  templateScale = parseFloat(templateWidth/savedWidth).toFixed(3);

  totals = parseFloat(dimesion).toFixed(3);
  return totals;
}