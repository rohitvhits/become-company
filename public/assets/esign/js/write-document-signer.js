...
var headers = [];
var TextArray = [];
var checkBoxDropdownArray = []
var radioDropdownArray = []
//Testing 
...
function sendRequest() {
	...
	if (isTemplate === "Yes" && templateFields !== '') {
		...
		docWidth = document.getElementById("document-viewer").getBoundingClientRect().width;
	}
	server({
		url: sendRequestUrl,
		data: {
...
function assemble(data, prepare) {
	...
	if (data.type === "rotate") {
	...
	} else if (data.type === "image" || data.type === "signature" || data.type === "symbol" || data.type === "shape" || data.type === "sign_verify") {
		if (data.group === "field") {
		...
		$(".signer-assembler").append('<action normalValueRadio="' + data.normalValueRadio + '" name="' + data.name + '" id="' + data.id + '" type="' + data.type + '" group="' + data.group + '" page="' + data.page + '" xPos="' + data.xPos + '" yPos="' + data.yPos + '" width="' + data.width + '" height="' + data.height + '" text="' + data.text + '" bold="' + data.InputId + '" italic="' + data.italic + '" font="' + data.font + '" fontsize="' + data.fontsize + '" value="' + data.value + '" datats="' + data.type + '" vishalpatel="' + data.required + '" ' + readOnly + ' background_color="' + data.background_color + '"  signer_id="' + data.signer_id + '" datas_keys="' + data.datas_keys + '" groups_checkbox="' + data.groups_checkbox + '" groupSmapleId="' + data.checkBoxsTemp + '" groupname="' + data.groupname + '">');
	}
...
$(".signer-save").click(function (event) {
	...
	// if (i === 0) {
		if (inviting) {
			...
			orgnizeData();
		} else if ($('.signer-assembler action').length || $('.signer-assembler action').length ==0) {
...
function orgnizeData(prepare) {
	...
	// Use getBoundingClientRect for viewport-relative coords (signer-elements are position:fixed)
	var viewerRect = document.getElementById("document-viewer").getBoundingClientRect();
	...
	$('.signer-builder .signer-element').each(function (index, value) {
		...
		if (actionType === "image" || actionType === "signature" || actionType === "symbol" || actionType === "shape") {
			...
			if (actionType === "symbol" || actionType === "shape") {
				...
				var contentEl = signerElement.find("svg")[0];
				textHolder = signerElement.find(".img_wrap");
			} else {
				thisImage = signerElement.find("img").attr('src');
				var contentEl = signerElement.find("img")[0];
				textHolder = signerElement.find(".img_wrap");
			}


			if(typeof textHolder.attr('dataids') === 'undefined'){
				toastr.error("A signature is required on page " + pageNumber + ". Please signature to continue.");
				stopOrganizing = true;
					return false
			}
			...
			elementXpos = contentRect?.left - viewerRect.left;
			elementYpos = contentRect?.top - viewerRect.top;
			var verified = 0;
			if(typeof textHolder.attr('data-show-verify') !="undefined"){
				verified = textHolder.attr('data-show-verify');
			}
			var signDoctorId = textHolder.attr('data-verify-doctor-id') || '';
			var signDoctorName = textHolder.attr('data-verify-doctor-name') || '';
			assemble({ "id": signerElement.attr('id'), "group": group, "type": actionType, "page": pageNumber, "xPos": elementXpos, "yPos": elementYpos, "width": elementWidth, "height": elementHeight, "image": thisImage, "tempArray": sessionArrays, "background_color": $(textHolder[0]).attr("background_color"), "signer_id": $(textHolder[0]).attr("signer_id"),'verified':verified, "doctor_id": signDoctorId, "doctor_name": signDoctorName }, false);

		}else if (actionType === "stamp") {

			if (group === "field") {
				if (signerElement.attr("stamped") === "false") {
					emptyAssembler();
					renderPage(pageNumber);
					signerElement.addClass("selected-element");
					notify("Hmm!", "A stamp is required on page " + pageNumber + ". Please stamp to continue.", "info", "Stamp Now");
					stopOrganizing = true;
					return false
				}
			}
			...
			thisImage = signerElement.find("img").attr('src');
			var stampContentEl = signerElement.find("img")[0];
			textHolder = signerElement.find(".img_wrap");

			if(typeof textHolder.attr('dataids') === 'undefined'){
				toastr.error("A stamp is required on page " + pageNumber + ". Please stamp to continue.");
				stopOrganizing = true;
					return false
			}
			var stampRect = stampContentEl.getBoundingClientRect();
			elementWidth = stampRect.width;
			elementHeight = stampRect.height;
			elementXpos = stampRect.left - viewerRect.left;
			elementYpos = stampRect.top - viewerRect.top;
		...
		}else if (actionType === "sign_verify") {

			thisImage = signerElement.find("img").attr('src');
			var verifyContentEl = signerElement.find("img")[0];
			textHolder = signerElement.find(".img_wrap");

			var verifyRect = verifyContentEl.getBoundingClientRect();
			elementWidth = verifyRect.width;
			elementHeight = verifyRect.height;
			elementXpos = verifyRect.left - viewerRect.left;
			elementYpos = verifyRect.top - viewerRect.top;
			var verifyDoctorId = signerElement.attr('data-doctor-id') || '';
			var verifyDoctorName = signerElement.attr('data-doctor-name') || '';
			assemble({ "id": signerElement.attr('id'), "group": group, "type": actionType, "page": pageNumber, "xPos": elementXpos, "yPos": elementYpos,"width": elementWidth, "height": elementHeight, "image": thisImage, "tempArray": sessionArrays, "doctor_id": verifyDoctorId, "doctor_name": verifyDoctorName}, false);
			...
			var textRect = textHolder[0].getBoundingClientRect();
			elementWidth = textRect.width;
			elementHeight = textRect.height;
			elementXpos = textRect.left - viewerRect.left;
			elementYpos = textRect.top - viewerRect.top;
			...
			var cbEl = signerElement.find(".checkbox_wrapper")[0];
			var cbRect = cbEl.getBoundingClientRect();
			elementWidth = cbRect.width;
			elementHeight = cbRect.height;
			elementXpos = cbRect.left - viewerRect.left;
			elementYpos = cbRect.top - viewerRect.top;
			...
			var dropRect = textHolders[0].getBoundingClientRect();
			elementWidth = dropRect.width;
			elementHeight = dropRect.height;
			elementXpos = dropRect.left - viewerRect.left;
			elementYpos = dropRect.top - viewerRect.top;
			var sessionArrays;
			readOnly = '';
...
function prepareData(save) {
	...
	$('.signer-assembler action').each(function (index, value) {
		...
		if ($(this).attr('type') == 'checkbox') {
			if (checkBoxDropdownArray[$(this).attr('name')] != 'undefined') {
				selectedObject = checkBoxDropdownArray[$(this).attr('name')];

				dynamicDropdownId =checkBoxDropdownArray[$(this).attr('name')]?.id;
			}
		}

		let selectedObjectVal = [];
		let dynamicDropdownIdVal = "";

		if ($(this).attr('type') == 'radio') {
			if (radioDropdownArray[$(this).attr('name')] != 'undefined') {
				selectedObjectVal = radioDropdownArray[$(this).attr('name')];
				
				dynamicDropdownIdVal =radioDropdownArray[$(this).attr('name')]?.id;
			}
		}
		...
		actions.push({
			...
			dynamicDropdownId: dynamicDropdownId,
			selectedObjectVal: selectedObjectVal,
			dynamicDropdownIdVal: dynamicDropdownIdVal,
			normalValue: $(this).attr('normalValue'),
			normalValueRadio: $(this).attr('normalValueRadio'),
			verified:$(this).attr('verified'),
			doctor_id: $(this).attr('doctor_id'),
			doctor_name: $(this).attr('doctor_name')
		});
...
function saveChanges(actions) {
	...
	swal({
		...
		if (isConfirm == false) {
			...
			showLoader();
			let vRect = document.getElementById("document-viewer").getBoundingClientRect();
			submitResponseWriteDocument(actions, vRect.width, document_key, signingKey, vRect.height);
		}
	})
}
...
$('.signstatus').click(function () {
    var zoomLevel = Math.round((window.outerWidth / window.innerWidth) * 100) || Math.round(window.devicePixelRatio * 100);

    if (zoomLevel === 100) { 
	sign = Math.floor(100000000 + Math.random() * 900000000) + new Date().getUTCMilliseconds();
	...
	$('<div class="signer-element selected-element tttts" resizeable="true" status="drop" type="image" id="sign_' + sign + '" page="' + pageNum + '"><img onclick="mySign(' + sign + ')" src="' + urls + 'assets/images/new_favicon_01.png" class="img_wrap" id="signatures_signer_' + sign + '"></div>').appendTo(".signer-builder");
	let ratio =100;
	$(document).mousemove(function (event) {
		$('#tempids').val(1);
		$('.prev').val('signature');

		$(".signer-element[status=drop]").css({ left: event.pageX + 1, top: event.pageY + 2 });
		var heights = ratio * 0.363636;
			$(".signer-element[status=drop]").css({ width: ratio,height: heights});
	});
	disableTools();
	highlightCanvas();
} else {
	swal({
		title: "Error!",
		text: "Drag and drop is disabled. Please set your zoom to 100%.",
		icon: "error",
		button: "OK",
	});
}
...
var sign_verify;
$('.signstatus_verify').click(function () {
    var zoomLevel = Math.round((window.outerWidth / window.innerWidth) * 100) || Math.round(window.devicePixelRatio * 100);

    if (zoomLevel === 100) { 
		sign_verify = Math.floor(100000000 + Math.random() * 900000000) + new Date().getUTCMilliseconds();

		deselectElements();
		$('.signstatus_verify').addClass('active');
		var check = $('.deselected').val();
		if (check != '') {
			$('.' + check).removeClass('active');
		}

		$('.deselected').val('signstatus_verify');
		$(`<div class="signer-element" type="sign_verify" status="drop" id="verify_${sign_verify}" page="${pageNum}">
			<div class="verifyImg">
				<img class="logo_img" src="https://web.nybestmedical.com/img/favicon.png" alt="Logo" style=" margin-right: 5px;">
				<div style="display: flex; flex-direction: column; justify-content: center;">
					<div style="color: rgb(40, 90, 140);">Verified by Nybest Portal</div>
					<div style="color: rgb(0, 150, 0);">${_VERIFIED_DATE_TIME}</div>
				</div>
				
			</div>
				<div class="elec_signer">
...
var stamp;
$('.stampstatus').click(function () {
	stamp = Math.floor(100000000 + Math.random() * 900000000) + new Date().getUTCMilliseconds();

	deselectElements();
	$('.stampstatus').addClass('active');
	var check = $('.deselected').val();
	if (check != '') {
		$('.' + check).removeClass('active');
	}

	$('.deselected').val('stampstatus');
	$('<div class="signer-element selected-element tttts" resizeable="true" status="drop" type="stamp" id="stamp_' + stamp + '" page="' + pageNum + '"><img onclick="myStamp(' + stamp + ')" src="' + urls + 'assets/images/stamp.jpg" class="img_wrap" id="stamp_signer_' + stamp + '"></div>').appendTo(".signer-builder");
	$(document).mousemove(function (event) {
		$('#tempids').val(1);
		$('.prev').val('stamp');
		$(".signer-element[status=drop]").css({ left: event.pageX + 1, top: event.pageY + 1 });
...
$('.datesigned').click(function () {
    ...
    if (zoomLevel === 100) {
	totalDateSign = Math.floor(100000000 + Math.random() * 900000000) + new Date().getUTCMilliseconds();
	deselectElements();
...
$('.text').click(function () {
    ...
    if (zoomLevel === 100) {
	totalText = Math.floor(100000000 + Math.random() * 900000000)  + new Date().getUTCMilliseconds();
...
$('.checkboxs').click(function () {
	...
	checkTotal = Math.floor(100000000 + Math.random() * 900000000)+new Date().getUTCMilliseconds();
...
$('.dropdowsns').click(function () {
	...
	drops = Math.floor(100000000 + Math.random() * 900000000) + new Date().getUTCMilliseconds();
...
$('.radios').click(function () {
	...
	var  radios = Math.floor(100000000 + Math.random() * 900000000)+new Date().getUTCMilliseconds();
...
$('.fields_caregiver').click(function () {
	...
	fields_caregiver = Math.floor(100000000 + Math.random() * 900000000) + new Date().getUTCMilliseconds();
...
$('.fields_staff').click(function () {
	...
	fields_staff = Math.floor(100000000 + Math.random() * 900000000) + new Date().getUTCMilliseconds();
...
function initElementsResize() {
	$(".signer-element[resizeable=true]").resizable({
		...
		resize: function (event, ui) {
			ui.helper.find("img").width('100%');
			ui.helper.find("img").height('100%');
		}
	});
...
function getResponse() {
	...
	if (texts == 'signstatus_verify' && sign_verify !== undefined) {

		var signname = '"signstatus_verify"';
		var localid = 'verify_' + sign_verify;
		groupname = localid;

		var verifyDoctorChecked = $('#verify_' + sign_verify).attr('data-doctor-id') ? "checked='checked'" : "";
		var verifyDoctorDisplay = verifyDoctorChecked ? "block" : "none";

		html = "<div class='row' id='" + sign_verify + "'><div class='box box-solid'><div class='box-body'><div id='" + sign_verify + "'><div class='form-group'><label>Verify By</label></div><hr><div class='col-md-12'><div class='form-group'><label>Signer</label><select name='changesColor' onchange='setSigner(this.id,\"signatures_signer_" + sign_verify + "\"," + signname + "," + sign_verify + ");' id='signer_signatures" + sign_verify + "' class='form-control'></select></div><hr></div><div class='col-md-12'><div class='form-group'><label>Data Label</label><div class=''>Verify By " + sign_verify + "</div></div><hr></div><div class='col-md-12'><div class='form-group'><label><input type='checkbox' class=''  name='signature" + sign_verify + "' id='check_sign_" + sign_verify + "' onclick='getGenerateArray(" + signname + "," + sign_verify + ");gerRequired(this.value," + sign_verify + "," + signname + ")' value='1' ></label>Required Field</div></div><div class='col-md-12'><div class='form-group'><input type='checkbox' class='minimal'  name='signature_read" + sign_verify + "' id='check_read_" + sign_verify + "' onclick='getGenerateArray(" + signname + "," + sign_verify + ");gerRequired(" + signname + "," + sign_verify + ")' value='1' >Read Only</div></div><div class='col-md-12'><div class='form-group'><label><input type='checkbox' class='' " + verifyDoctorChecked + " name='verify_doctor_check_" + sign_verify + "' id='verify_doctor_check_" + sign_verify + "' onclick='toggleVerifyDoctorDropdown(" + sign_verify + ")' value='1'> Verify By Doctor</label></div></div><div class='col-md-12' id='verify_doctor_wrapper_" + sign_verify + "' style='display:" + verifyDoctorDisplay + "'><div class='form-group'><label>Select Doctor</label><select name='verify_doctor_" + sign_verify + "' id='verify_doctor_" + sign_verify + "' class='form-control' onchange='setVerifyDoctor(" + sign_verify + ")'><option value=''>-- Select Doctor --</option></select></div></div></div></div>";
		ActionType = $('#' + localid).attr('type');
		page = $('#' + localid).attr('page');
		elementPosition = $('#' + localid).find("img").offset();
		elementWidth = $('#' + localid).find("img").width();
		...
		yposs = elementPosition?.top - viewerPositions.top;
		deleteid = $('#' + localid).attr('id');
		deleteids = sign_verify;
		var did = "'" + deleteid + "'";

		html += '<hr><a href="javascript:void(0)" onclick="getDeleteFields(' + did + ',' + deleteids + ')">Delete</a></div></div></div>';

		setTimeout(function() {
			if ($('#verify_doctor_check_' + sign_verify).is(':checked')) {
				loadDoctorDropdown(sign_verify);
			}
		}, 100);
	}

	if (texts == 'stamp' && stamp !== undefined) {
		var stampname = '"stamp"';
		var localid = 'stamp_' + stamp;
		groupname = localid;
		html = "<div class='row' id='" + stamp + "'><div class='box box-solid'><div class='box-body'><div id='" + stamp + "'><div class='form-group'><label>Stamp</label></div><hr><div class='col-md-12'><div class='form-group'><label>Signer</label><select name='changesColor' onchange='setSigner(this.id,\"stamp_signer_" + stamp + "\"," + stampname + "," + stamp + ");' id='signer_stamp" + stamp + "' class='form-control'></select></div><hr></div><div class='col-md-12'><div class='form-group'><label>Data Label</label><div class=''>Stamp " + sign + "</div></div><hr></div><div class='col-md-12'><div class='form-group'><label><input type='checkbox' class=''  name='stamp" + stamp + "' id='check_stamp_" + stamp + "' onclick='getGenerateArray(" + stampname + "," + stamp + ");gerRequired(this.value," + stamp + "," + stampname + ")' value='1' ></label>Required Field</div></div><div class='col-md-12'><div class='form-group'><input type='checkbox' class='minimal'  name='stamp_read" + sign + "' id='check_read_" + stamp + "' onclick='getGenerateArray(" + stampname + "," + stamp + ");gerRequired(" + stampname + "," + stamp + ")' value='1' >Read Only</div></div></div></div>";
		ActionType = $('#' + localid).attr('type');
		page = $('#' + localid).attr('page');
		elementPosition = $('#' + localid).find("img").offset();
		elementWidth = $('#' + localid).find("img").width();
		...
		html += '<hr><a style="margin-left:10px" href="javascript:void(0)" onclick="getDeleteFields(' + did + ',' + deleteids + ')">Delete</a></div></div></div>';
	}
	...
	if (texts == 'radios') {
		...
		if (lengths > 0) {
			$.each(RadioArray, function (index, valsd) {
				if (valsd.text != undefined) {
					textsBoxValue = valsd.text;
				} else {
					textsBoxValue = '';
				}

				OldTextArray[valsd.id] = textsBoxValue;
				if (valsd.checked == true) {
					chktrue = valsd.checked;
				} else {
					chktrue = false;
		...
		$.each($("input[name='" + groupname + "']"), function (index, elementObj) {
			...
			var ttmpName = "'radios'";
			checkBoxYsnamic += '<div class="mycheck' + radios + '' + i + '"><div class="row"><div class="form-group"><div class="col-md-2"><label for="inputEmail3" class="multiplecom"><input type="checkbox" ' + check + ' id="dymic_' + radios + "" + i + '" onclick="getOnClickRadio(' + ttmpName + ',' + subid + ',this.id,' + inputEmail + ',' + radios + ')"></label></div><div class="col-md-10"><input type="text" class="form-control W-50" id="inputEmail' + radios + i + '" value="' + textsd + '" placeholder="Radio value" onkeyup="getSetValueRadio(' + subid + ',this.id,' + radios + ')"><select id="radioContainer' + i + '" class="form-control radioContainerClass"  style="display: none;margin-bottom:4px"></select></div></div></div></div>';
			ctempid = i;
			i++;
...
function getAjax(id, keys) {
	...
	$.ajax({
		url: careginer,
		type: "GET",
		data: { 'value': keys, 'id': id, 'template_id': template_id },
		success: function (response) {
			if (response != '') {

				$('.caregiverId' + id).html(response).select2();
			}
		}
	});
}
	...
	if ($(".signer-element[status=drop]").length > 0) {
		if (event.target.id === "document-viewer") {
			...
			if(event.target.className !==""){
				toastr.error("You have moved outside the canvas area");
			//	notify("You have out of the canvas");
			}
		}
	...
	if (isTemplate === "Yes" && templateFields !== '' && $("body").hasClass("editor") && $(".signer-builder").is(':empty')) {
		...
		$.each(templateFields, function (i, field) {
			...
			if (field.type == "image") {
				...
				splitid = field.id.split('_');
				if (splitid[0] == 'stamp') {
					var ids = 'stamp_signer_' + splitid[1];
				} else {
					var ids = 'initial_signer_' + splitid[1];
				}

				$('<div class="signer-element " resizeable="true" id="' + field.id + '" type="stamp" group="field" page="' + field.page + '" style="display:none;left:' + xPos + 'px;top:' + yPos + 'px;position:absolute;"><img src="' + field.image + '" class="img_wrap"  id="' + ids + '" style="width:' + field.width + 'px;background-color:' + field.background_color + ';" background_color="' + field.background_color + '" signer_id="' + field.signer_id + '"></div>').appendTo(".signer-builder");
				initElementsDrag();
				...
				if(field.normalValue !=undefined){

				}else{
					field.normalValue = field.normal_value;

				}

				//xPos =parseFloat(xPos)-3;
				$('<div class="signer-element ui-draggable ui-draggable-handle"  tempid="' + field.groupSmapleId + '" id="' + field.id + '" type="checkbox"  group="input"   page="' + field.page + '"  style="position:absolute;display:none;left:' + xPos + 'px;top:' + yPos + 'px;' + colors + '"><input type="checkbox" class="checkbox_wrapper" contenteditable="true"  name="' + field.name + '" style=""  value="' + field.normalValue + '" ' + checked + ' id="' + field.bold + '"  background_color="' + field.background_color + '" signer_id="' + field.signer_id + '" ' + required + ' data-keys="' + field.addmore + '" group="' + field.groups_checkbox + '" ' + readonly + ' groupsName="' + field.groupNames + '" tempIds="' + field.groupSmapleId + '">' + onclicks + '</div>').appendTo(".signer-builder");
				...
				if(field.normalValueRadio !=undefined){

				}else{
					field.normalValueRadio = field.normal_value_radio;

				}

				$('<div class="signer-element ui-resizable ' + classes + '" tempid="' + field.groupSmapleId + '" id="' + field.id + '" type="radio"    page="' + field.page + '"  style="position:absolute;display:none;left:' + xPos + 'px;top:' + yPos + 'px;' + colors + 'background-color:' + field.background_color + ';"><input type="radio" id = "' + field.bold + '" class="radio_wrap"  value="' + field.normalValueRadio + '" ' + checked + ' name="' + field.name + '"  data-keys="' + field.addmore + '" ' + required + ' group="' + field.groups_checkbox + '"  background_color="' + field.background_color + '" ' + readonly + ' signer_id="' + field.signer_id + '"  groupsName="' + field.groupNames + '" tempIds="' + field.groupSmapleId + '">' + onclicks + '</div>').appendTo(".signer-builder");
				/* Logic conditional */
	...
	if ($("body").hasClass("accept") && requestPositions.length) {
		...
		$.each(requestPositions, function (i, field) {
			...
			if (field.type == "image") {
				...
				$('<div class="signer-element" type="stamp" stamped="false" group="field" page="' + field.page + '" style="display:none;left:' + xPos + 'px;top:' + yPos + 'px;position:absolute;"><img src="' + baseUrl + '/assets/images/stamp.jpg" style="width:' + signerScaler(field.width) + 'px;"></div>').appendTo(".signer-builder");
			} else if (field.type == "text") {
...
function getGenerateArray(key, val, temp = null) {
	...
	if (key == 'signature') {
		...
		var verifyOrNot =$('#signatures_signer_'+val).attr('data-show-verify');
		var verfifyChecked = "";
		if(verifyOrNot ==1){
			verfifyChecked = "checked='checked'";
		}
		var verifyBy="<input type='checkbox' class='sminimal' "+verfifyChecked+"  name='signature_verify" + val + "' id='signature_verify_" + val + "' onclick='showVerify(" + val + ")' value='1'>";
		var signDoctorId = $('#signatures_signer_'+val).attr('data-verify-doctor-id') || '';
		var signDoctorDisplay = (portalType =='Patient' && verifyOrNot == 1) ? "block" : "none";
		
		html = "<div class='row' id=" + val + " style=''><div class='box box-solid'><div class='box-body'><div id='" + val + "'><div class='form-group'> <label>Signature</label></div><hr><div class='col-md-12'><div class='form-group'><label>Data Label</label><div class=''>Signature " + val + "</div></div><hr></div><div class='col-md-12'><div class='form-group'><label>Show Verify By</label><div class=''>"+verifyBy+"</div></div></div><div class='col-md-12' id='sign_doctor_wrapper_" + val + "' style='display:" + signDoctorDisplay + "'><div class='form-group'><label>Select Doctor</label><select name='sign_verify_doctor_" + val + "' id='sign_verify_doctor_" + val + "' class='form-control' onchange='setSignatureVerifyDoctor(" + val + ")'><option value=''>-- Select Doctor --</option></select></div></div><hr><a href='javascript:void(0)' onclick='getDeleteFields(" + did + "," + deleteids + ")'>Delete</a></div></div></div>";
		setTimeout(function() {
			if (verifyOrNot == 1) {
				loadSignatureDoctorDropdown(val, signDoctorId);
			}
		}, 100);
	}
	...
	if (key == 'stamp') {
		...
		var stampname = '"stamp"';
		var selected = $('#signer_stamp' + val).html();
		var stampRequired = $('#check_stamp_' + val).prop('checked');
		if (stampRequired == true) {
			var stampname = '"stamp"';
			required = "<input type='checkbox' class='' name='stamp" + val + "' id='check_stamp_" + val + "' onclick='getGenerateArray('stamp'," + val + ")' value='1' checked='checked'>";
			stampRequired = 1;

		} else {
			required = "<input type='checkbox' class='' name='stamp" + val + "' id='check_stamp_" + val + "' onclick='getGenerateArray(stamp," + val + ")' value='1'>";
			stampRequired = 0;

		}
		var stampRead = $('#check_read_' + val).prop('checked');
		if (stampRead == true) {
			stampReadOnly = 1;
			readOnly = "<input type='checkbox' class='minimal'  name='stamp_read" + stamp + "' id='check_read_" + val + "' onclick='getGenerateArray(stamp," + val + ")' value='1' checked='checked'>";
		} else {
			stampReadOnly = 0;
			readOnly = "<input type='checkbox' class='minimal'  name='stamp_read" + val + "' id='check_read_" + val + "' onclick='getGenerateArray(stamp," + val + ")' value='1'>";
		}
		/*signer Seleted or not */
		html = "<div class='row' id=" + val + "><div class='box box-solid'><div class='box-body'><div id='" + val + "'><div class='form-group'> <label>Stamp</label></div><hr><div class='col-md-12'><div class='form-group'><label>Data Label</label><div class=''>Stamp " + val + "</div></div><hr></div><a style='margin-left:10px' href='javascript:void(0)' onclick='getDeleteFields(" + did + "," + deleteids + ")'>Delete</a></div></div></div>";
	}
	...
	if (key == 'text') {
		...
		if (portalType == 'Patient') {
			onBlurEvent = ' onblur="trimToMaxCharsNew(this,'+val+')" ';
		}

		html = "<div class='row' id=" + val + ">" +
			"<div class='box box-solid'>" +
			"<div class='box-body'>" +

			"<div id='" + val + "'>" +
			"<div class='form-group'>" +
			"<i class='fa fa-text-width' aria-hidden='true'></i>&nbsp;&nbsp;<label>Text</label>" +
			"</div><hr>" +
			...
			"<div>" +
			"<textarea class='textare" + val + "' " +
				"onkeyup='getKeys(" + val + ")' " +
				onBlurEvent + 
				"onchange='getGenerateArray(" + signname + "," + val + ")'>" +
				textareaDetails +
			"</textarea>" +
			"</div></div><hr></div>" +

			"<div class='col-md-12'>" +
			"<div class='form-group font-div'>" +
			"<label>Font</label>" +
			"<input type='text' id='font" + val + "' class='form-control font-size-box' " +
			"onkeyup='SetFontSize(" + val + "," + signname + ")' value='" + font + "'>" +
			"</div><hr></div>" +
			...
			"<a href='javascript:void(0)' onclick='getDeleteFields(" + localid + "," + deleteids + ")'>Delete</a>" +
	...
	if (key == 'checkbox' || key == "checkboxs") {
		...
		$.each($("input[name='" + groupname + "']"), function () {
			...
			var tmpName = "'checkbox'";
			checkBoxYsnamic += '<div class="mycheck' + val + '' + i + '"><div class="row"><div class="form-group"><div class="col-md-2"><label for="inputEmail3" class="multiplecom"><input type="checkbox" id="dymic_' + val + "" + i + '" ' + chknull + ' onclick="getOnClick(' + tmpName + ',' + subid + ',this.id,' + inputEmail + ',' + val + ')"></label></div><div class="col-md-10"><input type="text" class="form-control W-50 inputEmailClass" style="margin-bottom: 4px;" id="inputEmail' + val + i + '" value="' + tesztt + '" placeholder="Checkbox value" onkeyup="getSetValue(' + subid + ',this.id,' + val + ')"><select id="checkboxContainer' + i + '" class="form-control checkboxContainerClass" style="display: none;margin-bottom:4px" onchange="setCheckBoxDropdown(' + subid + ',this.id,' + val + ',' + i + '),getSetValue(' + subid + ',this.id,' + val + ')"></select></div></div></div></div>';
			ctempid = i;
			testid = val + i;
			i++;
	...
	if (key == 'radios' || key == 'radio') {
		...
		$.each($("input[name='radiogroup" + val + "']"), function () {
			...
			inputEmail = "'inputEmail" + val + i + "'";
			var tmpName = "'radios'";
			checkBoxYsnamic += '<div class="mycheck' + val + '' + i + '"><div class="row"><div class="form-group"><div class="col-md-2"><label for="inputEmail3" class="multiplecom"><input type="checkbox" id="dymic_' + val + "" + i + '" ' + chknull + ' onclick="getOnClickRadio(' + tmpName + ',' + subid + ',this.id,' + inputEmail + ',' + val + ')"></label></div><div class="col-md-10"><input type="text" class="form-control W-50 inputEmailClassRadio" id="inputEmail' + val + i + '" value="' + tesztt + '" placeholder="Radio value" onkeyup="getSetValueRadio(' + subid + ',this.id,' + val + ')"><select id="radioContainer' + i + '" class="form-control radioContainerClass" style="display: none;margin-bottom:4px" onchange="setRadioDropdown(' + subid + ',this.id,' + val + ',' + i + '),getSetValueRadio(' + subid + ',this.id,' + val + ')"></select></div></div></div></div>';
			ctempid = i;
			i++;
		});
		var setText = '"cid_' + val + ctempid + '"';
	...
	if (key == 'datesigned') {
		...
		html = "<div class='row' id=" + val + "><div class='box box-solid'><div class='box-body'><div id='" + val + "'><div class='form-group'><label>Date Signed</label></div><hr><div class='col-md-12'><div class='form-group'><label>Data Label</label> <div class=''> Date Signed " + val + "</div></div><hr><a href='javascript:void(0)' onclick='getDeleteFields(" + localid + "," + deleteids + ")'>Delete</a></div></div></div>";
	}
	
	if (key == 'signstatus_verify') {
		deleteids = val;
		var signname = '"signstatus_verify"';
		var localid = 'verify_' + val;
		var localidQuoted = '"' + localid + '"';

		var verifyDoctorChecked = $('#' + localid).attr('data-doctor-id') ? "checked='checked'" : "";
		var verifyDoctorDisplay = verifyDoctorChecked ? "block" : "none";
		var selectedDoctorId = $('#' + localid).attr('data-doctor-id') || '';
		if(portalType =='Patient'){
			html = "<div class='row' id='" + val + "'><div class='box box-solid'><div class='box-body'><div id='" + val + "'><div class='form-group'><i class='fa fa-check-square-o' aria-hidden='true'></i>&nbsp;&nbsp;<label>Verify By</label></div><hr><div class='col-md-12'><div class='form-group'><label><input type='checkbox' class='' " + verifyDoctorChecked + " name='verify_doctor_check_" + val + "' id='verify_doctor_check_" + val + "' onclick='toggleVerifyDoctorDropdown(" + val + ")' value='1'> Verify By Doctor</label></div></div><div class='col-md-12' id='verify_doctor_wrapper_" + val + "' style='display:" + verifyDoctorDisplay + "'><div class='form-group'><label>Select Doctor</label><select name='verify_doctor_" + val + "' id='verify_doctor_" + val + "' class='form-control' onchange='setVerifyDoctor(" + val + ")'><option value=''>-- Select Doctor --</option></select></div></div></div><a href='javascript:void(0)' onclick='getDeleteFields(" + localidQuoted + "," + deleteids + ")'>Delete</a></div></div></div>";
		}else{
			html = "<div class='row' id='" + val + "'><div class='box box-solid'><div class='box-body'><div id='" + val + "'><div class='form-group'><i class='fa fa-check-square-o' aria-hidden='true'></i>&nbsp;&nbsp;<label>Verify By</label></div><hr></div><a href='javascript:void(0)' onclick='getDeleteFields(" + localid + "," + deleteids + ")'>Delete</a></div></div></div>";
		}
		if(portalType =='Patient'){
			setTimeout(function() {
				if ($('#verify_doctor_check_' + val).is(':checked')) {
					loadDoctorDropdown(val, selectedDoctorId);
				}
			}, 100);
		}
	}
		...
		success: function (response) {
			if (response != '') {
				...
				$.each(json, function (index, value) {
					...
					if(value.dynamicDropdownId !=""){
						checkBoxDropdownArray[value.name] = [];
						
						checkBoxDropdownArray[value.name].id = value.dynamicDropdownId;
						
						var dpCount = 1;
						$.each(value.selectedObject,function(i,v){
							if(v !=null){
								checkBoxDropdownArray[value.name][dpCount] = v;
								dpCount++;
							}
							
						})
					}
					if(value.dynamicDropdownIdVal !=""){
						radioDropdownArray[value.name] = [];
						
						radioDropdownArray[value.name].id = value.dynamicDropdownIdVal;
						
						var dpCount = 1;
						$.each(value.selectedObjectVal,function(i,v){
							if(v !=null){
								radioDropdownArray[value.name][dpCount] = v;
								dpCount++;
							}
...
function getType(id, key, selectedValue) {
	...
	$.ajax({
		...
		success: function (response) {
			...
			if (key == 'stamp') {
				$('#signer_stamp' + id).html(" ");
				$('#signer_stamp' + id).append(response);
			}
			if (key == 'initial') {
...
var selectedWiseObjectResponse = [];

function getDynamicCheckbox(groupname) {
	$.ajax({
		url: getCheckbox,
		method: 'GET',
		data: { 'template_id': template_id },
		success: function (response) {
			var dropdownOption = "<option value=''>Select Checkboxes</option>";
			var checkboxData = Object.values(response);

			$.each(checkboxData, function (index, item) {
				var selected = "";
				var selectedValue = checkBoxDropdownArray[groupname]?.id;

				if (item.id == selectedValue) {
					selected = "selected";
				}
				dropdownOption += '<option value="' + item.id + '" ' + selected + '>' + item.label + '</option>'
				

			});
			var dropdownElementId = '#dynamicDropdown_' + groupname;

			$('#dynamicDropdown_' + groupname).html(dropdownOption);

			selectedWiseObjectResponse[groupname] = [];
			selectedWiseObjectResponse[groupname] = checkboxData;

			hideShowDropdownValue(groupname)
		},
		error: function () {
...
function hideShowDropdownValue(value, otherValue = "", subValue = "") {
	var drpValue = selectedWiseObjectResponse[value];
	var selectedValue = $('#dynamicDropdown_' + value).val();

	if (!checkBoxDropdownArray[value]?.id) {
		
		checkBoxDropdownArray[value] = [];
		checkBoxDropdownArray[value].id = selectedValue;
	}else{
		if(checkBoxDropdownArray[value].id != selectedValue){
			checkBoxDropdownArray[value].id = selectedValue;
		}
	}
	...
	$('input[name="'+value+'"]').each(function(){
		if (checkBoxDropdownArray[value]?.[cnt] != undefined) {

		} else {
			checkBoxDropdownArray[value][cnt] = '';
		}
	...
	$('.checkboxContainerClass').empty();
	if (selectedValue) {
		var selectedOptions = drpValue.find(function (item) {
			return item.id == selectedValue;
		});

		if (selectedOptions && selectedOptions.options.length > 0) {
			var optionsArray = JSON.parse(selectedOptions.options);
		
			
			$('.checkboxContainerClass').append($('<option>', {
				value: '',
				text: 'Select Values'
			}));
			...
			$.each(optionsArray, function (index, option) {
					var $option = '<option value="' + option + '" ' + selected + '>' + option + '</option>';
					$('.checkboxContainerClass').append($option);
			...
			$('input[name="'+value+'"]').each(function(i,v){
				var cnt = i+1;
				$('#checkboxContainer'+cnt).val(checkBoxDropdownArray[value][cnt]);
			
			})

			$('.checkboxContainerClass').show();
			$(".inputEmailClass").hide();


		}

	} else {
		$(".inputEmailClass").show();
		$('.checkboxContainerClass').empty();
		$('.checkboxContainerClass').hide();
	}

}

var selectedSubCheckboxId = [];

function setCheckBoxDropdown(subId, id, MainCheckboxId, i) {
	selectedSubCheckboxId = [];
	var explode = subId.split('_');
	var valueSelected = $('#' + id).val();
	selectedSubCheckboxId.push(valueSelected);
	checkBoxDropdownArray['cbox' + MainCheckboxId][i] = valueSelected;

}

var selectedWiseObjectResponseData = [];

function getDynamicRadio(groupname) {	
	$.ajax({
		url: getRadio,
		method: 'GET',
		data: { 'template_id': template_id },
		success: function (response) {
			var dropdownOption = "<option value=''>Select Radios</option>";
			var radioData = Object.values(response);
			...
			$.each(radioData, function (index, item) {
				...
				var selectedValue = radioDropdownArray[groupname]?.id;
			...
			var dropdownElementId = '#dynamicDropdownRadio_' + groupname;
			
			$('#dynamicDropdownRadio_' + groupname).html(dropdownOption);
			selectedWiseObjectResponseData[groupname] = [];
			selectedWiseObjectResponseData[groupname] = radioData;

			hideShowDropdownRadioValue(groupname);
...
function hideShowDropdownRadioValue(value, otherValue = "", subValue = "") {
	var drpValue = selectedWiseObjectResponseData[value];
	var selectedValue = $('#dynamicDropdownRadio_' + value).val();

	if (!radioDropdownArray[value]?.id) {
		
		radioDropdownArray[value] = [];
		radioDropdownArray[value].id = selectedValue;
	}else{
		if(radioDropdownArray[value].id != selectedValue){
			radioDropdownArray[value].id = selectedValue;
		}
	}
	...
	var cnt =1;
	$('input[name="'+value+'"]').each(function(){
		if (radioDropdownArray[value]?.[cnt] != undefined) {

		} else {
			radioDropdownArray[value][cnt] = '';
		}
	...
	if (selectedValue) {
		...
		if (selectedOptions && selectedOptions.options.length > 0) {
			...
			$('.radioContainerClass').append($('<option>', {
				value: '',
				text: 'Select Values'
			}));
			...
			$('input[name="'+value+'"]').each(function(i,v){
				var cnt = i+1;
				$('#radioContainer'+cnt).val(radioDropdownArray[value][cnt]);
			
			})

			$('.radioContainerClass').show();
			$(".inputEmailClassRadio").hide();

		}

	} else {
		$(".inputEmailClassRadio").show();
		$('.radioContainerClass').empty();
...
var selectedSubRadioId = [];

function setRadioDropdown(subId, id, MainCheckboxId, i) {
	selectedSubRadioId = [];
	var explode = subId.split('_');
	var valueSelected = $('#' + id).val();
	selectedSubRadioId.push(valueSelected);
	radioDropdownArray['radiogroup' + MainCheckboxId][i] = valueSelected;

}

function showVerify(id){
	$('#sign_verify_doctor_' + id).val('');
	$('#signatures_signer_' + id).removeAttr('data-verify-doctor-id');
	$('#signatures_signer_' + id).removeAttr('data-verify-doctor-name');
	var checked = $('#signature_verify_'+id).is(":checked");
	$('#sign_doctor_wrapper_' + id).hide();
	var status = 0;
	if(checked){
		status = 1;
	}
	...
	if(portalType =='Patient'){
		if(checked){
			$('#sign_doctor_wrapper_' + id).show();
			loadSignatureDoctorDropdown(id);
		} else {
			$('#sign_doctor_wrapper_' + id).hide();
		}
	}
	$('#signatures_signer_'+id).attr('data-show-verify',status);
	getGenerateArray('signature',id);
}

function loadSignatureDoctorDropdown(id, selectedDoctorId) {
	$.ajax({
		url: baseUrl + '/esign/load-doctor-list',
		type: 'GET',
		dataType: 'json',
		success: function(response) {
			if (response.status && response.data) {
				var dropdown = $('#sign_verify_doctor_' + id);
				dropdown.find('option:not(:first)').remove();
				$.each(response.data, function(index, doctor) {
					var selected = (selectedDoctorId && doctor.id == selectedDoctorId) ? ' selected' : '';
					dropdown.append('<option value="' + doctor.id + '"' + selected + '>' + doctor.full_name + '</option>');
				});
			}
		},
		error: function() {
			toastr.error('Failed to load doctor list');
		}
	});
}

function setSignatureVerifyDoctor(id) {
	var doctorId = $('#sign_verify_doctor_' + id).val();
	var doctorName = $('#sign_verify_doctor_' + id + ' option:selected').text();
	if (doctorId) {
		$('#signatures_signer_' + id).attr('data-verify-doctor-id', doctorId);
		$('#signatures_signer_' + id).attr('data-verify-doctor-name', doctorName);
	} else {
		$('#signatures_signer_' + id).removeAttr('data-verify-doctor-id');
		$('#signatures_signer_' + id).removeAttr('data-verify-doctor-name');
	}
...
function toggleVerifyDoctorDropdown(id) {
	var isChecked = $('#verify_doctor_check_' + id).is(':checked');
	$('#verify_doctor_' + id).val('');
	$('#verify_' + id).removeAttr('data-doctor-id');
	$('#verify_' + id).removeAttr('data-doctor-name');
	if(portalType =='Patient'){
		if (isChecked) {
			$('#verify_doctor_wrapper_' + id).show();
			loadDoctorDropdown(id);
		} else {
			$('#verify_doctor_wrapper_' + id).hide();
			
			$('#verify_' + id).find('.elec_signer').text('electronically signed');
			getGenerateArray('signstatus_verify', id);
		}
	}
	
}

function loadDoctorDropdown(id, selectedDoctorId) {
	$.ajax({
		url: baseUrl + '/esign/load-doctor-list',
		type: 'GET',
		dataType: 'json',
		success: function(response) {
			if (response.status && response.data) {
				var dropdown = $('#verify_doctor_' + id);
				dropdown.find('option:not(:first)').remove();
				$.each(response.data, function(index, doctor) {
					var selected = (selectedDoctorId && doctor.id == selectedDoctorId) ? ' selected' : '';
					dropdown.append('<option value="' + doctor.id + '"' + selected + '>' + doctor.full_name + '</option>');
...
function setVerifyDoctor(id) {
	var doctorId = $('#verify_doctor_' + id).val();
	var doctorName = $('#verify_doctor_' + id + ' option:selected').text();
	if (doctorId) {
		$('#verify_' + id).attr('data-doctor-id', doctorId);
		$('#verify_' + id).attr('data-doctor-name', doctorName);
		$('#verify_' + id).find('.elec_signer').text('electronically signed by ' + doctorName);
	} else {
		$('#verify_' + id).removeAttr('data-doctor-id');
		$('#verify_' + id).removeAttr('data-doctor-name');
		$('#verify_' + id).find('.elec_signer').text('electronically signed');
...
function trimToMaxChars(textarea) {
    ...
    if (val.length > MAX_CHARS) {
        let trimmed = val.substring(0, MAX_CHARS);
...
function showCharLimitWarning() {
    toastr.error('You have exceeded the character limit. Only '+MAX_CHARS+' characters are allowed. Additional text will not be saved.');
}
...
function trimToMaxCharsNew(textarea,id){
	let $textarea = $(textarea);
    let val = $textarea.val();
	let trimmed = val;
    if (val.length > MAX_CHARS) {
        trimmed = val.substring(0, MAX_CHARS);
        $textarea.val(trimmed);
        showCharLimitWarning();
    }
	$('#checks' + id).val(trimmed);
}