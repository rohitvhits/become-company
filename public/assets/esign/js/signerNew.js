...
function orgnizeData(prepare) {
  ...
  $('.signer-builder .signer-element').each(function (index, value) {
    ...
    if (actionType === "image" || actionType === "signature" || actionType === "symbol" || actionType === "shape") {
      ...
      assemble({ "id": signerElement.attr('id'), "name": eid[0].name, "required": textHolder[0].required, "readOnly": textHolder[0].readOnly, "group": group, "type": actionType, "page": pageNumber, "xPos": elementXpos, "yPos": elementYpos, "width": elementWidth, "height": elementHeight, "text": actionType, "value": checkedval, "InputId": eid[0].id, "background_color": $(eid[0]).attr("background_color"), "signer_id": $(eid[0]).attr("signer_id"), 'datas_keys': $(eid[0]).attr("data-keys"), "group": $(eid[0]).attr("group") }, false);
      ...
      var sessionArrays;
      assemble({ "id": signerElement.attr('id'), "required": textHolders[0].required, "readOnly": textHolders[0].readOnly, "group": group, "type": actionType, "page": pageNumber, "xPos": elementXpos, "yPos": elementYpos, "width": elementWidth, "height": elementHeight, "text": actionType, "tempArray": textHolders[0].innerHTML, "background_color": $(textHolder[0]).attr("background_color"), "signer_id": $(textHolder[0]).attr("signer_id"), 'addmoreArray': final_array }, false);
    }
...
function prepareData(save) {
  ...
  $('.signer-assembler action').each(function (index, value) {
    ...
    var AddmoDropDown = [];
    $.each(dropdownResponse, function (index, vals) {
      AddmoDropDown[vals.id] = vals.temps;
    ...
    if (explode[0] == 'caregiver') {
      names = 'caregiver';
      disabled = 'disabled';
    } else if (explode[0] == 'patient') {
      names = 'patient';
      disabled = 'disabled';
    } else if (explode[0] == 'intake') {
      names = 'intake';
      disabled = 'disabled';
    }
    ...
    actions.push({
      ...
      drops_valeus: AddmoDropDown[$(this).attr('id')]
      ...
      if (field.type == "image") {
        ...
        if (docusignId == field.signer_id) {
          ...
          if(field.signer_id !='StampUser' && field.signer_id != 'Patient'&& field.signer_id != 'Stamp' && field.user_type != 'Patient'){
            ...
            $('<div onclick="mySign(' + cnt + ')" class="signer-element"  type="image" page="' + field.page + '" style="left:' + xPos + 'px;top:' + yPos + 'px;position:absolute;display:none;" id="' + field.id + '" dataid="' + cnt + '"  page="' + pageNum + '"><img src="' + res + '" id="img' + cnt + '" style="opacity:0.5;height:'+height+'px;width:40px;' + bgrequired + '" background_color="' + field.background_color + '" signer_id="' + field.signer_id + '"></div>').appendTo(".signer-builder");
          } else if (field.signer_id == 'Patient' || field.signer_id == 'Sign') {
            var height = '40';
            if(typeof field.manualHeight !="undefined"){
              height = field.manualHeight;
            }
            $('<div class="signer-element"  type="image" page="' + field.page + '" style="left:' + xPos + 'px;top:' + yPos + 'px;position:absolute;display:none;" id="' + field.id + '" dataid="' + cnt + '"  page="' + pageNum + '"><img class="doctor-signature" dataids="1" src="" id="img' + cnt + '" style="opacity:1.5;height:'+height+'px;width:80px;' + bgrequired + '" background_color="' + field.background_color + '" signer_id="' + field.signer_id + '"><input type="hidden" name="doctor_signature" id="doctor_signature" value=""></div>').appendTo(".signer-builder");
            ...
            if(typeof field.manualHeight !="undefined"){
              ...
              $('<div class="signer-element"  type="image" page="' + field.page + '" style="left:' + xPos + 'px;top:' + yPos + 'px;position:absolute;display:none;" id="' + field.id + '" dataid="' + cnt + '"  page="' + pageNum + '"><img class="doctor-stamp" src="" dataids="1" id="img' + cnt + '" style="opacity:1.5;height:'+height+'px;width:80px;' + bgrequired + '" background_color="' + field.background_color + '" signer_id="' + field.signer_id + '"><input type="hidden" name="doctor_stamp" id="doctor_stamp" value=""></div>').appendTo(".signer-builder");
            } else {
              $('<div onclick="myStampUser(' + cnt + ')" class="signer-element"  type="image" page="' + field.page + '" style="left:' + xPos + 'px;top:' + yPos + 'px;position:absolute;display:none;" id="' + field.id + '" dataid="' + cnt + '"  page="' + pageNum + '"><img src="' + res + '" id="img' + cnt + '" style="opacity:0.5;height:30px;width:40px;' + bgrequired + '" background_color="' + field.background_color + '" signer_id="' + field.signer_id + '"></div>').appendTo(".signer-builder");
            }
          }
        ...
        if (field.temp1 != '' && field.temp3 != '') {
          ...
          $('<div class="signer-element"  page="' + field.page + '" style="position:absolute;display:none;left:' + xPos + 'px;top:' + yPos + 'px;z-index:99;"><input type="hidden" id="' + field.id + '" dataid="' + field.temp1 + '" ><label class="writing-pad1" id="int' + field.id + '" vishald style="font-size:' + fonts + 'px;' + marigns + ';width:' + widthss + '; ' + word_break + '"></label></div>').appendTo(".signer-builder");
        ...
        if (docusignId == field.signer_id) {
          ...
          $('<div class="signer-element"   id="' + field.id + '" type="text"    page="' + field.page + '" style="position:absolute;display:none;z-index:99;left:' + xPos + 'px;top:' + yPos + 'px;" ><select name="dropdownd" id="dropid' + explode[1] + '"  style="' + bgrequired + ';width:' + field.width + 'px;height:' + field.height + 'px;background-color:' + field.background_color + '" background_color="' + field.background_color + '" signer_id="' + field.signer_id + '" onchange="' + HideShowd + '">' + dropsdownList + '</select></div>').appendTo(".signer-builder");
          $.each(tempHeaderForHide[0], function (i, kes) {
...
function GetLoadComponents() {
  ...
  $.ajax({
    url: baseUrl + "/esign/template/esign-lookup-fields-new/" + document_key,
    type: "GET",
    ...
    success: function (response) {
      if (response != '') {
        ...
        $('.doctor-signature').attr('src', imagePath);
        $('#doctor_signature').val(signature);
        var imagePathStamp =json.doctor_stamp;
        $('.doctor-stamp').attr('src', imagePathStamp);
