					...
					@if($doctor_paper_list->total() != 0)
						...
						@foreach($doctor_paper_list as $row)
							<tr id="msgs-{{$row->id}}">
								...
								<td>
								<a onclick="viewLog({{$row->id}})" href="javascript:void(0)" title="View Log"  data-toggle="modal"  data-target="#view_log_html" data-whatever="@mdo" ><i class="fa fa-eye"></i></a>
									<a onclick="geteditDoctor({{$row->id}})" data-toggle="modal" data-target="#exampleModal-doc-paperedirediw" data-whatever="@mdo" href="javascript:void(0)"><i class="fa fa-edit"></i></a>
									<a onclick="getDeleteNotes({{$row->id}})"><i class="fa fa-trash-o"></i></a>
