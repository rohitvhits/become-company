		...
		<div class="row">
			<div class="col-12 grid-margin stretch-card">
				<div class="card">
					...
					<form action="<?php echo URL::to('/');?>/reminder/update" method="post" enctype="multipart/form-data" id="submitId">
					@csrf
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<input type="text" class="form-control" placeholder="Enter Title" id="first_name" name="title" value="<?php if(isset($query->title) && $query->title !='') { echo $query->title; } ?>">
							...
							<div class="col-md-6">
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<textarea name="description" class="form-control" rows="4" cols="50" id="description_id" placeholder="Description" value="<?php if(isset($query->title) && $query->message !='') { echo $query->message; } ?>"><?php if(isset($query->title) && $query->message !='') { echo $query->message; } ?></textarea>
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<input autocomplete="off" type="text" name="start_date" class="form-control" placeholder="Start Date" id="start_date" value="<?php if(isset($query->start_date) && $query->start_date !='') { echo date('m/d/Y',strtotime($query->start_date)); }?>">
							...
							<div class="col-md-6">
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<input autocomplete="off" type="text" name="end_date" class="form-control" id="end_date" placeholder="End Date" value="<?php if(isset($query->end_date) && $query->end_date !='') { echo date('m/d/Y',strtotime($query->end_date)); }?>">
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<input class="form-control" data-inputmask="'alias': 'datetime'" data-inputmask-inputformat="HH:MM" im-insert="false" value="<?php if(isset($query->start_time) && $query->start_time !=''){ echo date('H:i',strtotime($query->start_time));}?>">
										  <span class="error mt-2 text-danger" id="end_date_error"><?php echo $errors->agency->first('end_date');?></span>
									  </div>
								</div>
							</div>
									...
									<div class="col-sm-9">
										<select class="js-example-basic-multiple w-100" data-placeholder="Select assign to" name="assign_to[]" id="assign_to_id" multiple="multiple">
										  <?php 
											...
											if(isset($query->employee_id) && $query->employee_id !=''){
												$explode = explode(',',$query->employee_id);
												if(count($explode) >0){
													foreach($explode as $ls){
														$selectedUserArray[] = $ls;
													}
												}
											}
										  if(count($users) >0){
											  forEach($users as $vs){
												if(in_array($vs->user_type_fk,array(3,4))){
											  ?>
										  <option value="<?php echo $vs->id;?>" <?php if(in_array($vs->id,$selectedUserArray)){ echo "selected='selected'";}?>><?php echo $vs->name;?></option> 
										  <?php } } } ?>
										</select>
