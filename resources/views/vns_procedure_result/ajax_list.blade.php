    ...
    <tbody>
        ...
        @endphp
            ...
            @foreach($query as $val)
                <tr>
                <td>{{ $i}}</td>
                    <td class="mailbox-subject">{{ ucfirst($val->procedure_name ?? 'N/A')}}</td>
                    <td>{{ ucfirst($val->name)}}</td>
                    <td class="mailbox-date">{{ date('m/d/Y h:i A', strtotime($val->created_date)) }}<br>
                    {{ $val->first_name .' '.$val->last_name}}
                    </td>
                   <td>
                        @can('edit-vns-procedure-result')
                        <a href="javascript:void(0)" data-toggle="tooltip" title="Edit" onclick="getDetails('{{ $val->id}}')"><i class="fa fa-edit"></i></a>
                        @endcan
                        @can('delete-vns-procedure-result')
                        <a  href="javascript:void(0)"  data-toggle="tooltip" title="Delete" onclick="procedureResultDelete('{{ $val->id}}')"><i class="fa fa-trash"></i></a>
                        @endcan
                   </td>
