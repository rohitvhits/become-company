        ...
        <tbody>
            ...
            @if($va->is_flag_read == 0)
                @php
                    ...
                    $clickHtml = "makeFlagRead('" . addslashes($va->flag_id) . "', '". addslashes($va->patient_id) ."','Document');";
            ...
            @else
                @php
                    $clickHtml = "";
                    $href = url('patient/view/' . $va->patient_id);
                    $target = '_blank';
                @endphp
            @endif
            <tr>
            <div id="preasondoc{{ $key}}" style="display:none">
                ...
                <td nowrap>{{ $key+1 }}</td>
                <td>
                    <a target="{{$target}}" onclick="{{$clickHtml}}" href="{{$href}}"><?= '#' . '' . $va->patient_id ?></a>
                    @if($va->is_flag_read == 0)
                        <div style="position:relative"><span class="add_new_record left_record" >New</span></div>
                    @endif
                </td>
                <td nowrap>{{ $va->document_name }}</td>
                <td nowrap>{{ $va->request_service_id }}</td>
                <td nowrap>
                    @if ($va->attachment != '' && $va->deleted_flag == 'N')
                        <a target="_blank" href="{{ url('/dpp') }}/{{$va->id}}"><i class="fa fa-download"></i> Download</a>
                        <br>          
                ...
                <td nowrap>
                    ...
                    @if(!empty($va->services[0]))
                        @php $tempCountercc = 0; @endphp
                        @foreach($va->services as $srv)
                            @php $serviceArray[] = $srv->id; @endphp
                                <span class="badge {{ $names[$tempCountercc % count($names)] }} "> {{$srv->name}} </span>
                            @php $tempCountercc++; @endphp
                        @endforeach
                    @endif
                </td>
                <td nowrap>
                ...
                <td nowrap  title="{{ $va->reason }}">
                @php 
                $out = strlen($va->reason) > 50 ? substr($va->reason,0,50)."..." : $va->reason;
                @endphp
