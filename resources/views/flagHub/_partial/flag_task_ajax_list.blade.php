    ...
    <table id="order-listing1" class="table table-bordered table-head-fix recordtabletdwidth">
        ...
        <tbody>
            ...
            @if($val->is_flag_read == 0)
                @php
                    $clickHtml = "makeFlagRead('" . addslashes($val->flag_id) . "', '". addslashes($val->record_id) ."','Task');";
            ...
            @else
                @php
                    $clickHtml = "";
                    $href = url('hub-record/view/' . $val->record_id);
                    $target = '_blank';
                @endphp
            @endif
            <tr>
                <td nowrap>
                    <a target="{{$target}}" onclick="{{$clickHtml}}" href="{{$href}}"><?= '#' . '' . $val->record_id ?></a>
                    @if($val->is_flag_read == 0)
                ...
                <td title ="{{ $val->reasonNotes }}">
               @php 
                $out = strlen($val->reasonNotes) > 50 ? substr($val->reasonNotes,0,50)."..." : $val->reasonNotes;
                @endphp
               
                <a href="javascript:void(0)"  style="text-decoration: none; color: inherit;">{{$out}} </a></td>
            </tr>
            @endforeach
            @endif
            @if (count($query) == 0)
