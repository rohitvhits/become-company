<div class="modal fade modal-container" id="bulk-assign-task" tabindex="-1" role="dialog" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Bulk Assign Task<span class="" style="font-size: 18px;"><span id="task_id"></span></h4>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select name="bulk_assign_to_user_select" id="bulk_assign_to_user_select" placeholder="Select Assignee" class="form-control select2">
                                @if(!empty($user_list[0])) @foreach($user_list as $va)
                                <option value="{{$va->id}}">{{$va->name}}</option>
                                @endforeach @endif
                            </select>
                            <span id="bul_assign_user_id_error" class="error"></span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-success" onclick="bulkAssignTask();">Update</button>
