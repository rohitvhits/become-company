    ...
    <table id="order-listing1" class="table table-bordered table-width1">
        ...
        <tbody>
            ...
            <tr id="{{$val->id}}" class="taskBgHide">
                ...
                <td>
                    @if($val->record_id!='')
                    <a style="color:#01c100;font-weight:bold;" href="{{url('/hub-record/view/')}}/{{$val->record_id}}"># {{ $val->record_id}}</a>
                    @else
                ...
                <td>
                    ...
                    @endcan
                    @can('task-delete')
                        <a class="btn btn-danger mr-2 badge badge-danger" style="background-color:#cb0b0b" href="javascript:void(0)" onclick="deleteTask('{{$val->id}}','{{$val->record_id}}')"><i class="fa fa-trash"></i></a>
                    @endcan
                </td>
