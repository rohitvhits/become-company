    ...
    <div class="signer-overlay">
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="light-card document">
                    <div>
                        <span class="form-class">Form Elements</span>
                    </div>
                    <hr>
                    <div class="form-container">
                        @if (!empty($labels))
                            @php
                                $groupedLabels = collect($labels)->groupBy('page');
                            @endphp

                            @foreach ($groupedLabels as $page => $fields)
                                <div class="page-container">
                                    <div class="page-header dropdown-item" data-toggle="dropdown"
                                        data-page="{{ $page }}"
                                        onclick="generatePdfPage({{ $page }})">
                                        <span class="page-title">Page {{ $page }}</span>
                                        <span class="filled-count page-title">0</span><span
                                            class="page-title">/</span><span class="total-count page-title">0</span>
                                    </div>
                                    <div class="fields-container" style="display: none;"
                                        data-page="{{ $page }}">
                                        @foreach ($fields as $label)
                                            <div class="forms-card" data-id="{{ $label['id'] }}"
                                                data-name="{{ $label['name'] }}" data-type="{{ $label['type'] }}"
                                                data-filled="{{ $label['isFilled'] }}">
                                                <div class="forms-info">
                                                    <div>
                                                        <span class="label-id">{{ $label['label'] }}</span>
                                                        @if ($label['isRequired'] == 'true')
                                                            <span class="requiredField pull-right"><svg
                                                                    enable-background="new 0 0 32 32" height="13"
                                                                    viewBox="0 0 32 32" width="13"
                                                                    xmlns="http://www.w3.org/2000/svg">
                                                                    <path
                                                                        d="m26 32h-20c-3.314 0-6-2.686-6-6v-20c0-3.314 2.686-6 6-6h20c3.314 0 6 2.686 6 6v20c0 3.314-2.686 6-6 6z"
                                                                        fill="#ffe6e2"></path>
                                                                    <path
                                                                        d="m22.607 17.89-3.274-1.89 3.274-1.89c.319-.184.428-.592.244-.911l-1-1.732c-.184-.319-.592-.428-.911-.244l-3.274 1.89v-3.78c0-.368-.298-.667-.667-.667h-2c-.368 0-.667.298-.667.667v3.78l-3.274-1.89c-.319-.184-.727-.075-.911.244l-1 1.732c-.184.319-.075.727.244.911l3.276 1.89-3.274 1.89c-.319.184-.428.592-.244.911l1 1.732c.184.319.592.428.911.244l3.274-1.89v3.78c0 .368.298.667.667.667h2c.368 0 .667-.299.667-.667v-3.78l3.274 1.89c.319.184.727.075.911-.244l1-1.732c.182-.319.073-.727-.246-.911z"
                                                                        fill="#fc573b"></path>
                                                                </svg></span>
                                                        @endif
                                                        @if ($label['isFilled'] == '1' || $label['isFilled'] == '0')
                                                            <span class="pull-right">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="30"
                                                                    height="30" viewBox="0 0 64 32">
                                                                    <rect width="64" height="32"
                                                                        rx="5" fill="green"></rect>
                                                                    <text x="32" y="22" fill="white" font-size="18"
                                                                        font-family="Arial, sans-serif"
                                                                        text-anchor="middle">AUTO</text>
                                                                </svg>
                                                            </span>
                                                        @endif
        ...
        <div class="row" style=" margin-left:10px; margin-top:10px;float:left;width:100%;">
                    ...
                    <button type="submit" style="display:none;" id="finish_id" class="btn btn-danger finishButtons"
                        <?php
																													if ($document_all_details->status == 'Completed') { ?> disabled<?php } else { ?>onclick="submitResponse();"
                        <?php } ?> style="clear:both;display:none;float: left;">
                ...
                <div class="modal-body">
                    <div id="signature-pad" class="signature-pad">
                        <div class="signature-pad--body">
                            <div class="row">
                                <div class="col-md-4">
                                    <h4><input type="radio" name="typebod" checked onclick="showSignaturePad()"
                                            value="0" /> Draw </h4>
                                </div>
                                <div class="col-md-4">
                                    <h4><input type="radio" name="typebod" onclick="showTypePad()"
                                            value="1" />Type</h4>
                                </div>
                                <div class="col-md-4">
                                    <h4><input type="radio" name="typebod" onclick="showFileUpload()"
                                            value="2" /> Upload</h4>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    <?php } else { ?>
    var url = '{{ url("esign/aws-pdf-generate")}}?template_id=<?php echo $document_all_details->id; ?>',
    ...
    var isPatient = <?php echo isset($isPatient) && $isPatient ? 'true' : 'false'; ?>;
    var textLimitForPatient = 500;
</script>
...
<script src="{{ asset('assets/esign/js/signerNew1.js')}}?maNameIsKhan=<?php echo strtotime(now()); ?>"></script>
<script src="{{ asset('assets/esign/js/render_new.js')}}?keyurs=<?php echo strtotime(now()); ?>"></script>
<script src="{{ asset('assets/esign/js/custom.js')}}?keyurs=<?php echo strtotime(now()); ?>"></script>
...
<script>
    ...
    function submitResponse() {
        ...
        $.each(globalResponse, function(index, val) {
            ...
            if (val.type == "stamp" && ('<?php echo $sent_on; ?>' == val.signer_id)) {
                imgRequired = $("#img" + i).attr('dataids');
                width = '';
                height = '';
                value = $("#img" + i).attr('src');
            ...
            if ('<?php echo $sent_on; ?>' == val.signer_id || (val.temp1 == 'intake' || val.temp1 ==
...
{{-- Document Workflow: Submit with workflow action --}}
<script>
...
function savePatientDetails(){
    $.ajax({
        url: '{{ url("esign/document-workflow/streamlined") }}',
        type: "POST",
        data: {
            '_token': '{{ csrf_token() }}',
            'document_id': '<?php echo $document_report_id; ?>'
        },
