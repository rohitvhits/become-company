                        ...
                        <div class="card-body">
                            <div class="">
                                ...
                                @if (count($agency_all_form_data->agencyMaster) > 0)
                                    @foreach ($agency_all_form_data->agencyMaster->groupBy('form_group_id') as $formGroupId => $groupedFields)
                                        @if (isset($formGroupId))
                                        <fieldset>
                                            <legend>{{ $groupedFields->first()['formGroup']['title'] ?? '' }}</legend>
...
<script src="{{ asset('assets/modulejs/agency_all_form_table.js') }}?time={{ env('timestamps') }}"></script>
<script src="{{ asset('assets/modulejs/agency_all_form_table_view.js') }}?time={{ env('timestamps') }}"></script>
<script src="{{ asset('assets/modulejs/agency_all_form_download.js') }}?time={{ env('timestamps') }}"></script>
