    ...
    <tbody>
        ...
        @endphp
            ...
            @foreach($query as $val)
                <tr>
                <td>{{ $i}}</td>
                    <td class="mailbox-subject">{{ ucfirst($val->question_name)}}</td>
                    <td>{{ ucfirst($val->template_name ?? 'N/A')}}</td>
                    <td class="mailbox-date">{{ date('m/d/Y h:i A', strtotime($val->created_date)) }}<br>
                    {{ $val->first_name .' '.$val->last_name}}
                    </td>
                   <td>
                   @can('edit-vns-question')
                        <a href="javascript:void(0)" data-toggle="tooltip" title="Edit" onclick="getDetails('{{ $val->id}}')"><i class="fa fa-edit"></i></a>
                        @endcan
                        @can('delete-vns-question')
                        <a  href="javascript:void(0)"  data-toggle="tooltip" title="Delete" onclick="questionDelete('{{ $val->id}}')"><i class="fa fa-trash"></i></a>
                        @endcan
                   </td>
