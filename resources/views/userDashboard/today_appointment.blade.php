<table id="" class="table">
    ...
    <tbody>
        @if (count($todayAppoinmentData) > 0)
            ...
            <tr>
                <th scope="row"><a href="<?php echo URL::to('/'); ?>/patient/view/<?php echo $todayData->patient->id; ?>" target="_blank"><?= '#' . '' . $todayData->patient->id ?></a>
                </th>
                ...
                <?php
                    ...
                    <?php } ?>
                    @if ($todayData->status == '1st Attempt - Unable to Contact' || $todayData->status == '2nd Attempt - Unable to Contact' || $todayData->status == '3rd Attempt - Unable to Contact' || $todayData->status == 'Patient Asked to Reschedule' || $todayData->status == 'New Order Received')
                        <label for="" class='badge badge-info'>{{$todayData->status}}</label>
                    @endif

                    @if ($todayData->status == 'Telehealth Completed' || $todayData->status == 'Telehealth Completed , Pending Forms' || $todayData->status == 'Form Completed' || $todayData->status == 'Service Provided')
                        <label for="" class='badge badge-success'>{{$todayData->status}}</label>
                    @endif

                    @if ($todayData->status == 'Patient Deceased' || $todayData->status == 'Appointment was missed' || $todayData->status == 'Appointment Missed' || $todayData->status == 'Closed Temporarily')
                        <label for="" class='badge badge-danger'>{{$todayData->status}}</label>
                    @endif

                    @if ($todayData->status == 'Signed' || $todayData->status == 'Signed & Sent Back to the Agency' || $todayData->status == 'New Form Requested')
                        <label for="" class='badge badge-primary'>{{$todayData->status}}</label>
                    @endif
                </td>
            </tr>
            @endforeach
